...
const visibleActivity = "NOT EXISTS (SELECT 1 FROM pull_request_events WHERE pull_request_events.id=pull_timeline.entity_id AND pull_request_events.kind IN ('thread_resolved','thread_reopened'))";
...
export async function pullRevisionTimeline(env: Env, principal: Principal | null, pullId: string, sequence: number): Promise<PullRevisionWindow | null> {
  ...
  const result = next?.sequence
    ? await env.DB.prepare(`SELECT sequence,kind,entity_id AS entityId,created_at AS createdAt FROM pull_timeline WHERE pull_request_id=? AND ${visibleActivity} AND sequence>? AND sequence<? ORDER BY sequence`).bind(pullId, sequence, next.sequence).all<TimelineRow>()
    : await env.DB.prepare(`SELECT sequence,kind,entity_id AS entityId,created_at AS createdAt FROM pull_timeline WHERE pull_request_id=? AND ${visibleActivity} AND sequence>? ORDER BY sequence`).bind(pullId, sequence).all<TimelineRow>();
...
function timelineRows(env: Env, pullId: string, suffix: string): Promise<TimelineRow[]> {
  return env.DB.prepare(`SELECT sequence,kind,entity_id AS entityId,created_at AS createdAt FROM pull_timeline WHERE pull_request_id=? AND ${visibleActivity} ${suffix}`).bind(pullId).all<TimelineRow>().then((result) => result.results);
}

function revisionBoundaries(env: Env, pullId: string): Promise<RevisionBoundary[]> {
  return env.DB.prepare(`SELECT pull_request_events.kind,pull_timeline.sequence,users.handle AS actor,users.display_name AS actorDisplayName,pull_request_events.details,pull_request_events.created_at AS createdAt FROM pull_timeline JOIN pull_request_events ON pull_request_events.id=pull_timeline.entity_id JOIN users ON users.id=pull_request_events.actor_id WHERE pull_timeline.pull_request_id=? AND pull_timeline.kind='event' AND pull_request_events.kind IN ('commits_added','head_updated') ORDER BY pull_timeline.sequence`).bind(pullId).all<Omit<RevisionBoundary, 'details'> & { details: string }>().then((result) => result.results.map((boundary) => ({ ...boundary, details: parseDetails(boundary.details) })));
}
