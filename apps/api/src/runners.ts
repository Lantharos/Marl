...
export async function claimJob(env: Env, runner: Runner): Promise<Response> {
  ...
  type ClaimedJob = { id: string; stepsJson: string; environmentJson: string; artifactPathsJson: string; runtimeJson: string; leaseExpiresAt: string; runId: string; runNumber: number; runName: string; branch: string; commitId: string; repositoryId: string; untrusted: number; owner: string; repository: string };
  let leaseToken = '';
  ...
  for (const candidate of candidates.results) {
    ...
    const claimed = await env.DB.prepare(`UPDATE jobs SET state='running',runner_id=?,lease_token_hash=?,lease_expires_at=datetime('now','+45 seconds'),attempt=attempt+1,started_at=COALESCE(started_at,CURRENT_TIMESTAMP) WHERE id=? AND state='queued' AND EXISTS (SELECT 1 FROM runs WHERE runs.id=jobs.run_id AND runs.approval_required=0 AND runs.state IN ('queued','running')) RETURNING id`).bind(runner.id, tokenHash, candidate.id).first<{ id: string }>();
    if (!claimed) continue;
