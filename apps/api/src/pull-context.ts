...
export function pullSummary(row: PullRow, checks: CheckCounts = { total: 0, passed: 0, failed: 0, running: 0 }, reviewStatus: ReviewStatus = 'none', unresolved = 0) {
  ...
  return { id: row.id, number: row.number, repository: { owner: row.owner, name: row.repository }, title: row.title, author: row.author, authorDisplayName: row.authorDisplayName, authorAvatar: row.authorAvatarUrl, sourceRepository: { owner: row.sourceOwner, name: row.sourceRepository }, sourceBranch: row.sourceBranch, targetBranch: row.targetBranch, state, reviewStatus, checkSummary: checks, updatedAt: row.updatedAt };
}
