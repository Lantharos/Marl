...
export function pullSummary(row: PullRow, checks: CheckCounts = { total: 0, passed: 0, failed: 0, running: 0 }, reviewStatus: ReviewStatus = 'none', unresolved = 0) {
  ...
  return { id: row.id, number: row.number, repository: { owner: row.owner, name: row.repository }, title: row.title, author: row.author, authorAvatar: row.authorAvatarUrl, sourceBranch: row.sourceBranch, targetBranch: row.targetBranch, state, reviewStatus, checkSummary: checks, updatedAt: row.updatedAt };
}
...
export async function summarizePullRows(env: Env, rows: PullRow[]) {
  ...
  return rows.map((row) => {
    ...
    const value = pullSummary(row, counts, reviewStatus, unresolved);
