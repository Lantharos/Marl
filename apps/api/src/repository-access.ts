...
function accessibleRepositoryPredicate(alias: string, adminOnly = false) {
  ...
  return `(EXISTS (SELECT 1 FROM organization_members AS access_members WHERE access_members.organization_id=${alias}.organization_id AND access_members.user_id=? AND (access_members.role IN ('owner','admin') OR EXISTS (SELECT 1 FROM organizations AS access_organization WHERE access_organization.id=access_members.organization_id AND access_organization.base_repository_role ${adminOnly ? "='admin'" : 'IS NOT NULL'}))) OR EXISTS (SELECT 1 FROM repository_collaborators AS access_collaborators WHERE access_collaborators.repository_id=${alias}.id AND access_collaborators.user_id=? ${adminOnly ? "AND access_collaborators.role='admin'" : ''}) OR EXISTS (SELECT 1 FROM repository_team_grants AS access_grants JOIN team_members AS access_team_members ON access_team_members.team_id=access_grants.team_id JOIN organization_members AS access_team_organization_members ON access_team_organization_members.organization_id=${alias}.organization_id AND access_team_organization_members.user_id=access_team_members.user_id WHERE access_grants.repository_id=${alias}.id AND access_team_members.user_id=? ${adminOnly ? "AND access_grants.role='admin'" : ''}))`;
}

export function repositoryListFilter(principal: Principal, alias = 'repositories', { includeDeleted = false, adminOnly = false }: { includeDeleted?: boolean; adminOnly?: boolean } = {}) {
