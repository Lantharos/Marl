...
import { pullChecksApproval } from './pull-checks';
...
import { authorizeRepository, repositoryListFilter, repositoryCan } from './repository-access';
...
export async function listPulls(env: Env, principal: Principal | null, owner: string, name: string, url: URL): Promise<Response> {
  ...
  const labelsSql = labelFilterSql(labels);
  const after = cursor ? 'AND (pull_requests.updated_at<? OR (pull_requests.updated_at=? AND pull_requests.id<?))' : '';
  const querySql = search.query ? `AND (pull_requests.title LIKE ? ESCAPE '\\' OR pull_requests.source_branch LIKE ? ESCAPE '\\' OR pull_requests.number=?)` : '';
  const filters = [repository.id, ...(state !== 'open' && state !== 'all' ? [state] : []), ...labels.map((label) => label.toLowerCase()), ...(labels.length ? [labels.length] : []), ...(search.query ? [search.like, search.like, Number(search.query.replace(/^[#!]/, '')) || -1] : [])];
  const values = cursor ? [...filters, cursor.value, cursor.value, cursor.id, limit + 1] : [...filters, limit + 1];
  const [rows, availableLabels] = await Promise.all([
    env.DB.prepare(`${pullSelect} WHERE pull_requests.repository_id=? ${stateSql} ${labelsSql} ${querySql} ${after} ORDER BY pull_requests.updated_at DESC,pull_requests.id DESC LIMIT ?`).bind(...values).all<PullRow>(),
...
export async function getPull(env: Env, principal: Principal | null, owner: string, name: string, number: number): Promise<Response> {
  ...
  const [reviews, checks, unresolvedThreads, rule, commits, labels, availableLabels, assignees, availableAssignees, timeline, linkedItems, checksApproval] = await Promise.all([
  ...
  return json({ pullRequest: { ...pullSummary, state, body: pull.body, sourceCommitId: pull.sourceCommitId, targetCommitId: pull.targetCommitId, authorId: pull.authorId, createdAt: pull.createdAt, mergedCommitId: pull.mergedCommitId, mergeMethod: pull.mergeMethod, mergeRequirements: requirements, allowedMergeMethods: rule.allowedMergeMethods, commits: commits.results, checks: checks.results, labels: labels.results, availableLabels: availableLabels.results, assignees: assignees.results, availableAssignees: availableAssignees.results, locked: Boolean(pull.lockedAt), canManage: repositoryCan(repository, principal, 'repository.triage'), canMerge: permission.allowed, checksApproval, canModerate: repositoryCan(repository, principal, 'repository.maintain'), realtimeVersion: Number(pull.realtimeVersion), linkedItems, timeline } });
}
...
export async function getPullState(env: Env, principal: Principal | null, owner: string, name: string, number: number): Promise<Response> {
  ...
  const permission = pullMergePermission(repository, principal, pull.authorId, rule);
  const requirements = mergeRequirements(pull, rule, checkSummary, reviews.results, Number(unresolvedThreads?.count ?? 0), permission.authorMerge);
  const pullSummary = summary(pull, checkSummary, reviewStatusFor(pull, reviews.results), Number(unresolvedThreads?.count ?? 0));
  const state = pull.state === 'open' ? (requirements.ready ? 'mergeable' : 'blocked') : pullSummary.state;
  return json({ state: { state, sourceCommitId: pull.sourceCommitId, targetCommitId: pull.targetCommitId, mergedCommitId: pull.mergedCommitId, mergeMethod: pull.mergeMethod, commits: commits.results, reviewStatus: pullSummary.reviewStatus, checkSummary, mergeRequirements: requirements, canMerge: permission.allowed, allowedMergeMethods: rule.allowedMergeMethods, checksApproval, linkedItems, realtimeVersion: Number(pull.realtimeVersion) } });
}
...
export async function getPullPatch(env: Env, principal: Principal | null, owner: string, name: string, number: number, url: URL): Promise<Response> {
  ...
  const revision = requestedRevision && requestedRevision !== pull.sourceCommitId
    ? await env.DB.prepare(`SELECT json_extract(details,'$.head') AS head,json_extract(details,'$.base') AS base FROM pull_request_events WHERE pull_request_id=(SELECT id FROM pull_requests WHERE repository_id=? AND number=?) AND kind IN ('commits_added','head_updated') AND json_extract(details,'$.head')=? ORDER BY created_at DESC LIMIT 1`).bind(repository.id, number, requestedRevision).first<{ head: string; base: string }>()
