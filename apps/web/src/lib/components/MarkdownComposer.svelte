<script lang="ts">
  ...
  let { value = $bindable(''), placeholder = 'Leave a comment', minHeight = 120, compact = false, context } = $props<{ value?: string; placeholder?: string; minHeight?: number; compact?: boolean; context?: MarkdownContext }>();
...
<div class="composer" class:compact>
  ...
  {#if !compact}<footer><span>Markdown supported</span></footer>{/if}
</div>

<style>
  .composer{overflow:hidden;border:1px solid var(--border);border-radius:7px;background:var(--surface);transition:border-color 120ms ease}.composer:has(textarea:focus){border-color:var(--brand)}header{display:flex;min-height:38px;align-items:center;justify-content:space-between;border-bottom:1px solid var(--border-subtle);background:var(--surface-muted)}.modes{display:flex;height:38px;align-items:center}.modes :global(.mode.button){position:relative;height:38px;border-radius:0;padding:0 13px;font-size:10px}.modes :global(.mode.button.active){color:var(--text-strong)}.modes :global(.mode.button.active:after){position:absolute;right:10px;bottom:-1px;left:10px;height:2px;background:var(--brand);content:''}.tools{display:flex;align-items:center;padding-right:7px}.tools :global(.button){width:29px;height:29px}textarea{display:block;width:100%;padding:12px;border:0;outline:0;resize:vertical;background:var(--surface);color:var(--text);font-family:inherit;font-size:12px;line-height:1.6}.preview{padding:12px;font-size:12px;line-height:1.6}.preview p{margin:0;color:var(--text-faint);font-size:12px}footer{display:flex;min-height:27px;align-items:center;padding:0 11px;border-top:1px solid var(--border-subtle);color:var(--text-faint);font-size:8px}.compact header,.compact .modes,.compact .modes :global(.mode.button){height:34px;min-height:34px}.compact textarea,.compact .preview{padding:10px 11px}.compact .tools :global(.button){width:27px;height:27px}
</style>
