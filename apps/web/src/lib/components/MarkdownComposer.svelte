...
<div class="composer">
  ...
  {#if mode === 'write'}<textarea bind:this={textarea} bind:value {placeholder} aria-label={placeholder} style:min-height={`${minHeight}px`}></textarea>{:else}<div class="preview" style:min-height={`${minHeight}px`}>{#if value.trim()}<MarkdownBody source={value} {context} />{:else}<p>Nothing to preview</p>{/if}</div>{/if}
...
<style>
  ...
  footer{display:flex;min-height:28px;align-items:center;padding:0 14px;color:var(--text-faint);font-size:10px}
