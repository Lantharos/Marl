...
<article class="thread" class:inline class:outdated={thread.outdated}>
  <header><a href="#file-{thread.path.replaceAll('/','-')}">{thread.path}:{thread.line}</a><div>{#if thread.outdated}<span>Outdated</span>{:else if thread.resolved}<span class="resolved"><Check size={11} />Resolved</span><button disabled={busy} onclick={() => onResolve(thread.id, false)}>Reopen</button>{:else}<button disabled={busy} onclick={() => onResolve(thread.id, true)}>Resolve conversation</button>{/if}</div></header>
