...
<article class="thread" class:inline class:outdated={thread.outdated}>
  <header><a href="#file-{thread.path.replaceAll('/','-')}">{thread.path}:{thread.startLine === thread.line ? thread.line : `${thread.startLine}–${thread.line}`}</a><div>{#if thread.outdated}<span>Outdated</span>{:else if thread.resolved}<span class="resolved"><Check size={11} />Resolved</span><button disabled={busy} onclick={() => onResolve(thread.id, false)}>Reopen</button>{:else}<button disabled={busy} onclick={() => onResolve(thread.id, true)}>Resolve conversation</button>{/if}</div></header>
...
<style>
  .thread{overflow:hidden;border:1px solid var(--border);border-radius:8px;background:var(--surface)}.thread.inline{margin:10px}.thread.outdated{opacity:.65}.thread>header{display:flex;min-height:39px;align-items:center;justify-content:space-between;padding:0 11px;border-bottom:1px solid var(--border-subtle);background:var(--surface-muted)}header>a{color:var(--text-muted);font:9px ui-monospace,SFMono-Regular,Consolas,monospace;text-decoration:none}header>a:hover{color:var(--brand)}header>div{display:flex;align-items:center;gap:7px}header span{color:var(--warning);font-size:9px}header .resolved{display:flex;align-items:center;gap:4px;color:var(--success)}button{border:0;background:transparent;color:var(--text-muted);cursor:pointer;font-size:9px}header button{height:25px;padding:0 8px;border:1px solid var(--border);border-radius:5px;background:var(--surface)}header button:hover{border-color:var(--border-strong);color:var(--text-strong)}.entry{padding:11px 13px}.entry+.entry{border-top:1px solid var(--border-subtle)}.meta{display:flex;align-items:center;gap:7px;margin-bottom:9px}.avatar{display:grid;width:23px;height:23px;place-items:center;border-radius:50%;background:#d5b496;color:#3d2518;font-size:7px;font-weight:750}.meta strong{color:var(--text-strong);font-size:10px}.meta time{color:var(--text-faint);font-size:8px}.actions{display:flex;align-items:center;gap:2px;margin-left:auto}.actions button{display:grid;min-width:25px;height:25px;padding:0 6px;place-items:center;border-radius:5px}.actions button:hover{background:var(--surface-hover);color:var(--text)}.actions .confirm{background:var(--danger-soft);color:var(--danger)}.deleted{margin:0;color:var(--text-faint);font-size:10px;font-style:italic}.entry footer,.reply-composer footer{display:flex;justify-content:flex-end;gap:6px;margin-top:7px}.entry footer button,.reply-composer footer button{height:29px;padding:0 9px;border:1px solid var(--border);border-radius:5px;background:var(--surface)}button.primary{border-color:var(--brand)!important;background:var(--brand)!important;color:white!important}.reply{display:flex;align-items:center;gap:5px;margin:0 11px 10px;padding:6px 8px;border-radius:5px}.reply:hover{background:var(--surface-hover);color:var(--text-strong)}.reply-composer{padding:11px;border-top:1px solid var(--border-subtle)}
</style>
