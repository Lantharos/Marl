  ...
  <div class="filters" aria-label="Filters">
    ...
    {#if labelOptions.length || selectedLabels.length}
      ...
      <div class="label-picker" use:dismissable={() => (labelsOpen = false)}>
        ...
        {#if labelsOpen}
          <div class="label-menu" transition:popoverMotion>
...
<style>
  .filter-bar{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:22px}.filters,.tabs,.selected-labels{display:flex;min-width:0;align-items:center;gap:4px}.filters{flex-wrap:wrap}.divider{width:1px;height:18px;margin:0 5px;background:var(--border)}.label-picker{position:relative}.label-picker :global(.add-label.button){border:0;border-radius:999px;background:var(--surface-raised)}.label-picker :global(.add-label.button:hover){border-color:var(--border-strong)}.label-menu{position:absolute;z-index:80;top:35px;left:0;width:min(270px,calc(100vw - 48px));padding:7px;border:0;border-radius:15px;background:var(--surface-raised);box-shadow:var(--shadow-popover);transform-origin:top left}.label-search,.query{display:flex;align-items:center;gap:7px;border:1px solid var(--border);border-radius:8px;background:var(--surface);color:var(--text-faint)}.label-search{height:36px;padding:0 10px}.query{width:min(280px,42%);height:36px;padding:0 11px}.label-search:focus-within,.query:focus-within{border-color:var(--brand)}input{min-width:0;width:100%;border:0;outline:0;background:transparent;color:var(--text-strong);font-size:12px}input::placeholder{color:var(--text-faint)}.label-options{display:grid;max-height:260px;overflow-y:auto;margin-top:6px;gap:2px}.label-options :global(.label-option.button){height:auto;min-height:38px;justify-content:space-between;padding:7px 8px;border-radius:9px;text-align:left}.label-copy{display:flex;min-width:0;align-items:center;gap:8px}.label-copy>b{width:9px;height:9px;flex:0 0 auto;border-radius:50%}.label-copy strong,.label-copy small{display:block;overflow:hidden;max-width:190px;text-overflow:ellipsis;white-space:nowrap}.label-copy strong{color:var(--text-strong);font-size:12px}.label-copy small{margin-top:2px;color:var(--text-faint);font-size:11px}.label-options>p{margin:0;padding:18px 8px;color:var(--text-faint);font-size:11px;text-align:center}@media(max-width:680px){.filter-bar{align-items:stretch;flex-direction:column}.query{width:100%}.selected-labels{flex-wrap:wrap}}
</style>
