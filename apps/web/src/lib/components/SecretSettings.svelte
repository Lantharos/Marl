...
<form onsubmit={(event) => { event.preventDefault(); void save(); }}>
  ...
  <button disabled={busy || !name || !value}>{busy ? 'Saving…' : 'Add secret'}</button>
</form>
{#if error}<p class="error" role="alert">{error}</p>{/if}
<section>
  {#each secrets as secret}
    <article><span class="icon"><KeyRound size={16} /></span><span><strong>{secret.name}</strong><small>Updated <Time value={secret.updatedAt} /></small></span><button aria-label="Delete {secret.name}" onclick={() => remove(secret)}><Trash2 size={15} /></button></article>
...
<style>
  .page-head{padding-bottom:24px;border-bottom:1px solid var(--border-subtle)}h1{margin:0;color:var(--text-strong);font-size:25px;letter-spacing:-.03em}.page-head p{max-width:650px;margin:7px 0 0;color:var(--text-muted);font-size:13px;line-height:1.5}form{display:grid;grid-template-columns:210px minmax(0,1fr) auto;align-items:end;gap:12px;padding:24px 0;border-bottom:1px solid var(--border)}label{display:grid;gap:7px}label span{color:var(--text-strong);font-size:12px;font-weight:630}input{height:38px;padding:0 10px;border:1px solid var(--border-strong);border-radius:6px;outline:0;background:var(--surface);color:var(--text-strong);font-size:13px}input:focus{border-color:var(--brand)}button{display:inline-flex;height:38px;align-items:center;justify-content:center;padding:0 13px;border:1px solid var(--brand);border-radius:6px;background:var(--brand);color:white;cursor:pointer;font-size:12px;font-weight:640}button:disabled{opacity:.5}.error{padding:10px;border-radius:6px;background:var(--danger-soft);color:var(--danger);font-size:12px}article{display:grid;grid-template-columns:36px minmax(0,1fr) 38px;align-items:center;gap:10px;min-height:68px;border-bottom:1px solid var(--border-subtle)}.icon{display:grid;width:32px;height:32px;color:var(--text-muted);place-items:center}article strong,article small{display:block}article strong{color:var(--text-strong);font-size:13px}article small{margin-top:4px;color:var(--text-muted);font-size:11px}article button{border-color:transparent;background:transparent;color:var(--danger)}.empty{padding:54px 0;color:var(--text-muted);text-align:center}.empty strong{display:block;margin-top:8px;color:var(--text-strong);font-size:14px}.empty p{font-size:12px}@media(max-width:700px){form{grid-template-columns:1fr}form button{justify-self:start}}
</style>
