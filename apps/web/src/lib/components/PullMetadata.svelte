  ...
  <div class="field" use:dismissable={() => open === 'assignees' && (open = null)}>
    ...
    {#if open === 'assignees'}
      <div class="picker" bind:this={pickerPanel}>
        ...
        <div class="options">{#each matchingAssignees as person (person.id)}<Button class="option" variant="ghost" block disabled={busy} onclick={() => toggleAssignee(person.id)}><span class="option-main"><UserAvatar name={person.displayName || person.handle} src={person.avatarUrl} size={23} /><span><strong>{person.displayName || person.handle}</strong><small>@{person.handle}</small></span></span>{#if pull.assignees.some((item: PullRequestPerson) => item.id === person.id)}<Check size={14} />{/if}</Button>{:else}<p>No matching people</p>{/each}</div>
      </div>
    {/if}
  </div>
...
<style>
  .metadata{display:grid;gap:24px;margin-top:25px}.field{position:relative}.field>header{display:flex;height:27px;align-items:center;justify-content:space-between}.field>header>span{display:flex;align-items:center;gap:6px;color:var(--text-muted);font-size:11px;font-weight:630}.people{display:grid;gap:7px;margin-top:7px}.people :global(.user-profile-link){font-size:11px}.labels{display:flex;flex-wrap:wrap;gap:5px;margin-top:7px}.labels>span:not(.empty){display:inline-flex;align-items:center;gap:5px;padding:4px 7px;border-radius:5px;background:color-mix(in srgb,var(--label-color) 15%,transparent);color:var(--label-color);font-size:11px;font-weight:650}.labels i{width:6px;height:6px;border-radius:50%;background:currentColor}.empty{color:var(--text-faint);font-size:11px}.picker{position:fixed;z-index:90;overflow-y:auto;border:1px solid var(--border-strong);border-radius:9px;background:var(--surface-raised);box-shadow:var(--shadow-card)}.search{display:flex;align-items:center;gap:7px;margin:7px;padding:0 9px;border:1px solid var(--border);border-radius:6px;color:var(--text-faint)}.search:focus-within{border-color:var(--brand)}.search input{min-width:0;width:100%;height:32px;border:0;outline:0;background:transparent;color:var(--text-strong);font-size:11px}.options{max-height:310px;overflow-y:auto;padding:0 5px 5px;border-top:1px solid var(--border-subtle)}.options :global(.option.button){height:auto;min-height:45px;justify-content:space-between;padding:7px 8px;text-align:left;white-space:normal}.option-main{display:flex;min-width:0;align-items:center;gap:8px}.option-main>b{width:10px;height:10px;flex:0 0 auto;border-radius:50%}.option-main strong,.option-main small{display:block}.option-main strong{color:var(--text-strong);font-size:11px}.option-main small{overflow:hidden;max-width:220px;margin-top:2px;color:var(--text-faint);font-size:11px;text-overflow:ellipsis;white-space:nowrap}.options :global(.create-option.button){height:auto;min-height:42px;justify-content:flex-start;margin-top:3px;padding:7px 8px;border-top:1px solid var(--border-subtle);font-size:11px;white-space:normal}.options :global(.create-option strong){color:var(--text-strong)}.options>p{margin:0;padding:20px 8px;color:var(--text-faint);font-size:11px;text-align:center}.conversation-state{display:flex;min-height:34px;align-items:center;justify-content:space-between;color:var(--text-muted);font-size:11px}.conversation-state>span{display:flex;align-items:center;gap:6px}
</style>
