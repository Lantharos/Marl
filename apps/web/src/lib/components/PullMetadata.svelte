...
<section class="metadata">
  <div class="field" use:dismissable={() => (assigneesOpen = false)}>
    ...
    <div class="values">{#each pull.assignees as person}<UserProfileLink handle={person.handle} displayName={person.displayName || person.handle} avatarUrl={person.avatarUrl} size={21} />{:else}<span class="empty">No one assigned</span>{/each}</div>
