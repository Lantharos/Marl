...
<style>
  .confirmation{display:grid;gap:13px}.passkey,.loading{display:flex;align-items:center;gap:11px;padding:11px;border-radius:7px;background:var(--surface);color:var(--text-muted)}.passkey>:global(svg),.loading>:global(svg){flex:0 0 auto;color:var(--brand)}.passkey strong{display:block;color:var(--text-strong);font-size:11px}.passkey p{margin:4px 0 0;color:var(--text-muted);font-size:11px;line-height:1.45}.loading{font-size:11px}label{display:grid;gap:8px}label span{color:var(--text-strong);font-size:11px;font-weight:630}input{height:37px;padding:0 9px;border:1px solid var(--border-strong);border-radius:6px;outline:0;background:var(--surface);color:var(--text-strong);font:inherit;font-size:12px}input:focus{border-color:var(--brand)}.error{margin:0;padding:9px 10px;border-radius:6px;background:var(--danger-soft);color:var(--danger);font-size:11px}
</style>
