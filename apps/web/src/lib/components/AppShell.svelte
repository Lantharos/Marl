  ...
  <header class="workbar" use:dismissable={() => (mobileOpen = false)}>
    ...
    <div class="actions">
      ...
      <div class="menu-anchor" use:dismissable={() => (profileOpen = false)}>
        <button class="avatar-button" aria-label="Account menu" aria-expanded={profileOpen} onclick={() => { profileOpen = !profileOpen; createOpen = false; }}>{#if user.avatarUrl}<img src={user.avatarUrl} alt="" />{:else}{initial}{/if}</button>
        {#if profileOpen}<div class="popover profile-menu"><div>{#if user.avatarUrl}<img class="avatar" src={user.avatarUrl} alt="" />{:else}<span class="avatar">{initial}</span>{/if}<span><strong>{user.displayName}</strong><small>@{user.handle}</small></span></div><a href="/settings/account/profile" onclick={() => (profileOpen = false)}><span><strong>Settings</strong><small>Profile, security, and developer access</small></span></a><button onclick={toggleTheme}>{#if theme === 'dark'}<Sun size={15} />Light appearance{:else}<Moon size={15} />Dark appearance{/if}</button><button onclick={signOut}>Sign out</button></div>{/if}
      </div>
