...
<section class="list">
  {#each issues as issue (issue.id)}
    <article class="row">
      ...
      <span class="main">
        <a class="title" href="/{issue.repository.owner}/{issue.repository.name}/issues/{issue.number}">{issue.title}</a>
        ...
        {#if issue.labels.length}<span class="labels">{#each issue.labels.slice(0, 4) as label (label.id)}<span style:--label-color={label.color}>{label.name}</span>{/each}{#if issue.labels.length > 4}<i>+{issue.labels.length - 4}</i>{/if}</span>{/if}
      </span>
...
<style>
  .list{display:grid;gap:4px}.row{position:relative;display:grid;grid-template-columns:32px minmax(0,1fr) auto;align-items:center;gap:11px;min-height:76px;padding:10px 11px;border-radius:8px;transition:background-color 120ms ease}.row:hover{background:var(--surface-hover)}.state-icon{display:grid;width:30px;height:30px;place-items:center;border-radius:7px;background:var(--success-soft);color:var(--success)}.state-icon.closed{background:var(--surface-muted);color:var(--text-faint)}.main{min-width:0}.title,.main>small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.title{color:var(--text-strong);font-size:12px;font-weight:650;text-decoration:none}.title::after{position:absolute;inset:0;content:''}.main>small{margin-top:4px;color:var(--text-muted);font-size:10px}.author,.repository{position:relative;z-index:1;color:var(--text-strong);text-decoration:none}.author:hover,.repository:hover{color:var(--brand)}.labels{position:relative;z-index:1;display:flex;gap:5px;margin-top:6px}.labels span{padding:3px 6px;border-radius:999px;background:color-mix(in srgb,var(--label-color) 15%,transparent);color:var(--label-color);font-size:8px;font-weight:650}.labels i{color:var(--text-faint);font-size:8px;font-style:normal}.end{display:flex;align-items:center;gap:10px}.assignees{position:relative;z-index:1;display:flex}.assignees :global(.avatar){margin-left:-5px;border:2px solid var(--canvas)}.comments{display:flex;align-items:center;gap:4px;color:var(--text-faint);font-size:9px}.empty{padding:52px 20px;color:var(--text-faint);text-align:center}.empty strong{display:block;margin-top:10px;color:var(--text-strong);font-size:13px}.empty p{margin:5px 0 0;font-size:10px}@media(max-width:600px){.assignees,.labels{display:none}.row{grid-template-columns:30px minmax(0,1fr) auto;padding-inline:5px}}
</style>
