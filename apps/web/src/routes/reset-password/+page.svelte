...
<AuthShell title={complete ? 'Password updated' : 'Choose a new password'} description={complete ? 'Your old password can no longer be used.' : 'Use at least 12 characters and do not reuse a password from another service.'} {footer}>{#if !complete}<form class="auth-form" onsubmit={(event) => { event.preventDefault(); void reset(); }}>{#if error}<p class="auth-error">{error}</p>{/if}<label class="auth-field"><span>New password</span><input type="password" autocomplete="new-password" minlength="12" bind:value={password} required /></label><label class="auth-field"><span>Confirm password</span><input type="password" autocomplete="new-password" minlength="12" bind:value={confirm} required /></label><button class="auth-submit" disabled={busy}>Update password</button></form>{/if}</AuthShell>
