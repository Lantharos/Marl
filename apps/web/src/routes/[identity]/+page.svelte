...
{:else if organization}
  <main class="organization-page">
    <header class="hero"><OrganizationAvatar name={organization.name} src={organization.avatarUrl} size={92} /><div><p class="slug">{organization.slug}</p><h1>{organization.name}</h1>{#if organization.description}<p class="description">{organization.description}</p>{/if}<div class="meta">{#if organization.website}<a href={organization.website} target="_blank" rel="noreferrer"><ExternalLink size={13} />{websiteLabel}</a>{/if}<span><CalendarDays size={13} />Created {new Date(organization.createdAt).toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}</span></div></div>{#if canManage}<LinkButton class="manage" href="/organizations/{organization.slug}/settings/profile"><Settings size={13} />Organization settings</LinkButton>{/if}</header>
