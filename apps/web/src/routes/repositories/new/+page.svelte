...
<FormShell title="Create a repository" description="A new home for code, reviews, and self-hosted automation.">
  <form class="form-grid" onsubmit={(event) => { event.preventDefault(); void createRepository(); }}>
    <div class="field-row"><label class="field"><span>Owner</span><Select bind:value={owner} ariaLabel="Repository owner" options={[{ value: 'lantharos', label: 'lantharos' }]} /></label><label class="field"><span>Repository name</span><input bind:value={name} placeholder="new-project" autocomplete="off" /><small>Use a short, memorable name.</small></label></div>
    ...
    <div class="form-actions"><a href="/repositories">Cancel</a><button type="submit" disabled={!name.trim() || submitting}>{submitting ? 'Creating…' : 'Create repository'}</button></div>
  </form>
</FormShell>
