<script lang="ts">
  ...
  async function createRepository() {
    if (!nameValid || submitting) return; submitting = true; error = '';
    try { await api('/repositories', { method: 'POST', body: JSON.stringify({ owner, name: submittedName, description: description.trim(), visibility }) }); await goto(`/${owner}/${submittedName}`); }
...
<style>.form-error{margin:0;padding:9px 11px;border-radius:8px;background:var(--danger-soft);color:var(--danger);font-size:11px}</style>
