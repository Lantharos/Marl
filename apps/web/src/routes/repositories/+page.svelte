<script lang="ts">
  ...
  let items = $state<RepositorySummary[]>([]);
  ...
  onMount(async () => {
    try { items = (await api<{ repositories: RepositorySummary[] }>('/repositories')).repositories; }
