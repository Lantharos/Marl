<script lang="ts">
  ...
  let pulls = $state<PullRequestSummary[]>([]);
  ...
  onMount(async () => {
    const results = await Promise.allSettled([
      api<{ pullRequests: PullRequestSummary[] }>('/pulls'),
      api<{ repositories: RepositorySummary[] }>('/repositories'),
      api<{ runs: RunSummary[] }>('/runs'),
      api<{ runners: RunnerSummary[] }>('/runners')
    ]);
    if (results[0].status === 'fulfilled') pulls = results[0].value.pullRequests;
    if (results[1].status === 'fulfilled') repositories = results[1].value.repositories;
    if (results[2].status === 'fulfilled') runs = results[2].value.runs;
    if (results[3].status === 'fulfilled') runners = results[3].value.runners;
    unavailable = results.some((result) => result.status === 'rejected'); loading = false;
  });
</script>
...
<main class="page">
  <header class="hello"><div><h1>Hey, Kristof.</h1><p>{#if loading}Checking in on your work…{:else if attentionCount > 0}{attentionCount} {attentionCount === 1 ? 'thing needs' : 'things need'} you.{:else}You’re clear for now.{/if}</p></div><div class="quick"><a href="/repositories/new"><Plus size={14} />Repository</a><a href="/pulls/new"><GitPullRequest size={14} />Pull request</a></div></header>
