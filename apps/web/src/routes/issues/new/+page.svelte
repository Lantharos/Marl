<script lang="ts">
  ...
  async function create() { if (creating || uploading || title.trim().length < 3 || !repository) return; creating = true; error = ''; try { const result = await api<{ issue: { number: number } }>(`/repositories/${parts.owner}/${parts.name}/issues`, { method: 'POST', body: JSON.stringify({ title, body }) }); await goto(`/${parts.owner}/${parts.name}/issues/${result.issue.number}`); } catch (cause) { error = cause instanceof MarlApiError ? cause.message : 'Issue could not be created.'; creating = false; } }
</script>
