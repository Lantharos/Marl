...
<div class="key-list">
  {#each sshKeys as key}
    <article><span class="key-icon"><KeyRound size={17} /></span><div><strong>{key.name}</strong><code>{key.fingerprint}</code><small>Added <Time value={key.createdAt} />{#if key.lastUsedAt} · last used <Time value={key.lastUsedAt} />{:else} · never used{/if}</small></div><button aria-label={`Remove ${key.name}`} onclick={() => removeKey(key)}><Trash2 size={15} /></button></article>
