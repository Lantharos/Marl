...
<header class="page-head"><h2>Profile</h2></header>
{#if error}<p class="error" role="alert">{error}</p>{/if}
<form class="profile-form" onsubmit={(event) => { event.preventDefault(); void save(); }}>
  <section class="avatar-section"><ImageUploadButton state={avatarState} label="Change profile picture" size={64} round onclick={() => avatarInput?.click()}><UserAvatar name={displayName || username} src={avatarUrl} size={72} /></ImageUploadButton><div><strong>Profile picture</strong><p>PNG, JPEG, or WebP · up to 2 MB</p><input bind:this={avatarInput} class="avatar-input" type="file" accept="image/png,image/jpeg,image/webp" onchange={chooseAvatar} /></div></section>
...
<style>
  .page-head{padding-bottom:25px;}h2{margin:0;color:var(--text-strong);font-size:23px;letter-spacing:-.03em}.avatar-section p{margin:6px 0 0;color:var(--text-muted);font-size:11px;line-height:1.5}.error{display:flex;align-items:center;gap:7px;padding:9px 10px;border-radius:8px;background:var(--danger-soft);color:var(--danger);font-size:11px}.profile-form{padding:24px;border-radius:16px;background:var(--surface);box-shadow:var(--shadow-surface)}.avatar-section{display:flex;align-items:center;gap:16px;padding:0 0 8px;}.avatar-section strong{color:var(--text-strong);font-size:12px}.avatar-input{display:none}.fields{display:grid;grid-template-columns:1fr 1fr;gap:22px;padding:24px 0}.fields label:nth-child(n+3){grid-column:1/-1}.fields label{display:grid;gap:7px}.fields label>span{color:var(--text-strong);font-size:11px;font-weight:630}.fields input,.fields textarea{width:100%;padding:9px;border:1px solid var(--border);border-radius:8px;outline:0;background:var(--canvas);color:var(--text-strong);font-size:13px}.fields input{height:38px}.fields textarea{resize:vertical;line-height:1.5}.fields input:focus,.fields textarea:focus{border-color:var(--brand)}.fields small{justify-self:end;color:var(--text-faint);font-size:11px}.username{display:grid;grid-template-columns:34px 1fr}.username>span{display:grid;place-items:center;border:1px solid var(--border);border-right:0;border-radius:8px 0 0 6px;background:var(--surface-muted);color:var(--text-muted);font-size:11px}.username input{border-radius:0 6px 6px 0}footer{display:flex;justify-content:flex-end;padding-top:16px;}
@media(max-width:600px){.fields{grid-template-columns:1fr}.profile-form{padding:20px}.avatar-section{align-items:flex-start}}
</style>
