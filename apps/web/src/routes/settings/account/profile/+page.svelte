...
<form onsubmit={(event) => { event.preventDefault(); void save(); }}>
  <section class="avatar-section"><ImageUploadButton state={avatarState} label="Change profile picture" size={72} round onclick={() => avatarInput?.click()}>{#snippet children()}<UserAvatar name={displayName || username} src={avatarUrl} size={72} />{/snippet}</ImageUploadButton><div><strong>Profile picture</strong><p>Click your picture to change it. PNG, JPEG, or WebP up to 2 MB.</p><input bind:this={avatarInput} class="avatar-input" type="file" accept="image/png,image/jpeg,image/webp" onchange={chooseAvatar} /></div></section>
