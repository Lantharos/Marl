<script lang="ts">
  ...
  let query = $state('');
  const items = $derived(data.branches as Array<{ name: string; commit: string; title: string; updatedAt: string; isDefault: boolean; ahead: number; behind: number }>);
