...
<nav class="crumbs"><a href="{base}/code">{$page.params.repo}</a><span>/</span>{#each filePath.split('/') as part, index}<a href="{base}/{index === filePath.split('/').length - 1 ? 'blob' : 'tree'}/{revisionPath}/{encodeRepositoryPath(filePath.split('/').slice(0,index+1).join('/'))}">{part}</a>{#if index < filePath.split('/').length - 1}<span>/</span>{/if}{/each}</nav>
...
<style>
  .crumbs { display: flex; align-items: center; gap: 6px; margin-bottom: 12px; color: var(--text-faint); font-size: 11px; } .crumbs a { color: var(--brand); font-weight: 570; text-decoration: none; } .file-head { display: flex; min-height: 46px; align-items: center; justify-content: space-between; padding: 0 10px 0 13px; border: 1px solid var(--border); border-radius: 8px 8px 0 0; background: var(--surface-muted); } .file-head > div { display: flex; align-items: center; gap: 7px; } .file-head strong { color: var(--text-strong); font-size: 11px; } .file-head span { color: var(--text-faint); font-size: 9px; }
