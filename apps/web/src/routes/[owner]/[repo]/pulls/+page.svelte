<script lang="ts">
  ...
  const filteredItems = $derived(items.filter((pull) => {
    ...
    return stateMatches && `${pull.title} ${pull.author} ${pull.sourceBranch} ${pull.targetBranch} ${pull.labels.map((label) => label.name).join(' ')}`.toLowerCase().includes(query.trim().toLowerCase());
  }));
</script>
