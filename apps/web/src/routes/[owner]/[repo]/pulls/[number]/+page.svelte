<script lang="ts">
  ...
  async function createLineComment(draft: { path: string; side: 'old' | 'new'; startLine: number; line: number }, body: string) {
    if (!body.trim() || busy) return; busy = true;
    try { await api(`/repositories/${owner}/${repo}/pulls/${number}/threads`, { method: 'POST', body: JSON.stringify({ ...draft, startSide: draft.side, body }) }); await load(); }
    catch (cause) { error = cause instanceof StyApiError ? cause.message : 'Comment could not be added.'; }
    finally { busy = false; }
  }
  ...
  async function updateMetadata(body: { assigneeIds?: string[]; labelIds?: string[]; locked?: boolean }) {
    if (busy) return; busy = true; error = '';
    try { await api(`/repositories/${owner}/${repo}/pulls/${number}/metadata`, { method: 'PATCH', body: JSON.stringify(body) }); await load(); }
    ...
    <div class="conversation-layout">
      <main class="timeline">
        ...
        {#each timelineItems() as item}
          ...
          {/if}
        {/each}
        {#if pull.locked}<div class="locked-note"><CircleAlert size={14} />This conversation is locked. Unlock it from the sidebar to continue discussing this pull request.</div>{:else}<CommentComposer bind:value={commentBody} avatar="K" placeholder="Leave a comment" {busy} onSubmit={addPullComment} />{/if}
      </main>
