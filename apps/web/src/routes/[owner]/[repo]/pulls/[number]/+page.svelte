    ...
    <div class="conversation-layout">
      <main class="timeline">
        ...
        {#each timelineItems() as item}
          ...
          {/if}
        {/each}
        <PullActionComposer bind:value={commentBody} pullState={pull.state} ready={pull.mergeRequirements.ready} locked={pull.locked} {busy} onComment={addPullComment} onAction={composerAction} />
      </main>
      <aside class="sidebar"><section class="merge-panel">
        ...
        {#if pull.state !== 'merged'}
          ...
          {#if pull.state !== 'closed' && pull.state !== 'draft'}<div class="merge-choice"><Select bind:value={mergeMethod} ariaLabel="Merge method" options={pull.allowedMergeMethods.map((method: MergeMethod) => ({ value: method, label: method === 'merge' ? 'Merge commit' : method === 'squash' ? 'Squash and merge' : 'Rebase and merge' }))} /></div>{/if}
        {/if}
