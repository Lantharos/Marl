...
{:else}
  ...
  {#if tab === 'conversation'}
    <div class="conversation-layout">
      <main class="timeline">
        <article class="comment"><header><span class="avatar">{pull.author.slice(0,2).toUpperCase()}</span><strong>{pull.author}</strong><span>opened this pull request</span><Time value={pull.createdAt} /></header><div><MarkdownBody source={pull.body || 'No description was provided.'} /></div></article>
        {#each timeline.order as key, index (key)}
          ...
          {:else if item.kind === 'review'}
            <article class="event"><span class="event-icon {item.value.state}">{#if item.value.state === 'approved'}<CircleCheck size={15} />{:else if item.value.state === 'changes_requested'}<CircleAlert size={15} />{:else}<MessageSquare size={15} />{/if}</span><div><p><strong>{item.value.author}</strong> {item.value.state === 'approved' ? 'approved these changes' : item.value.state === 'changes_requested' ? 'requested changes' : 'reviewed this pull request'} <Time value={item.value.createdAt} /></p>{#if item.value.body}<div class="event-body"><MarkdownBody source={item.value.body} /></div>{/if}</div></article>
  ...
  {:else if tab === 'commits'}
    <section class="commit-list">{#each pull.commits as commit}<a href="/{owner}/{repo}/commit/{commit.id}"><span class="commit-mark"><GitCommitHorizontal size={14} /></span><span><strong>{commit.title}</strong><small>{commit.author} · <Time value={commit.authoredAt} /></small></span><code>{commit.shortId}</code></a>{:else}<div><strong>No commits to merge</strong><p>The target branch already contains this pull request head.</p></div>{/each}</section>
  {:else if tab === 'changes'}
