...
{:else}
  ...
  <nav class="tabs" aria-label="Pull request sections">
    <Button class={`tab${tab === 'conversation' ? ' active' : ''}`} variant="ghost" onclick={() => selectTab('conversation')}><MessageSquare size={15} />Conversation <span>{timeline.total}</span></Button>
    <Button class={`tab${tab === 'commits' ? ' active' : ''}`} variant="ghost" onclick={() => selectTab('commits')}><GitCommitHorizontal size={15} />Commits <span>{pull.commits.length}</span></Button>
    <Button class={`tab${tab === 'changes' ? ' active' : ''}`} variant="ghost" onclick={() => selectTab('changes')}><FileDiff size={15} />Changes {#if diff}<span>{diff.files.length}</span>{/if}</Button>
    <Button class={`tab${tab === 'checks' ? ' active' : ''}`} variant="ghost" onclick={() => selectTab('checks')}><CircleCheck size={15} />Checks <span>{pull.checks.length}</span></Button>
  </nav>
...
<style>
  ...
  .tabs{display:flex;height:42px;margin-bottom:20px;border-bottom:1px solid var(--border);gap:3px}.tabs :global(.tab.button){position:relative;height:42px;padding:0 11px;border-radius:0;font-size:11px;font-weight:580}.tabs :global(.tab.button::after){position:absolute;inset:auto 7px -1px;height:2px;background:transparent;content:''}.tabs :global(.tab.button.active){color:var(--text-strong)}.tabs :global(.tab.button.active::after){background:var(--brand)}.tabs span{padding:1px 5px;border-radius:99px;background:var(--surface-muted);color:var(--text-faint);font-size:9px}.action-error{display:grid;grid-template-columns:18px minmax(0,1fr) 30px;align-items:center;gap:6px;margin:-8px 0 14px;padding:8px 8px 8px 11px;border-left:2px solid var(--danger);border-radius:0 6px 6px 0;background:var(--danger-soft);color:var(--danger);font-size:10px}
  ...
  @media(max-width:600px){.title-row{display:block}.state{margin-bottom:8px}.title-row h1{font-size:20px}.tabs :global(.tab.button){padding:0 8px}.tabs span{display:none}.conversation-layout{gap:12px}}
</style>
