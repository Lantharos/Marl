...
{:else}
      ...
      <main class="timeline">
        ...
        {#each timeline.order as key, index (key)}
          ...
          {:else if item.kind === 'review'}
            <article class="event"><span class="event-icon {item.value.state}">{#if item.value.state === 'approved'}<CircleCheck size={15} />{:else if item.value.state === 'changes_requested'}<CircleAlert size={15} />{:else}<MessageSquare size={15} />{/if}</span><div><p><strong>{item.value.author}</strong> {item.value.state === 'approved' ? 'approved these changes' : item.value.state === 'changes_requested' ? 'requested changes' : 'reviewed this pull request'} <Time class="end" value={item.value.createdAt} /></p>{#if item.value.body}<div class="event-body"><MarkdownBody source={item.value.body} /></div>{/if}</div></article>
  ...
  {:else if tab === 'commits'}
    <section class="commit-list">{#each pull.commits as commit}<a href="/{owner}/{repo}/commit/{commit.id}"><span class="commit-mark"><GitCommitHorizontal size={14} /></span><span><strong>{commit.title}</strong><small>{commit.author} · <Time value={commit.authoredAt} />{#if commit.signatureStatus === 'verified'}<i><BadgeCheck size={12} />Verified</i>{/if}</small></span><code>{commit.shortId}</code></a>{:else}<div><strong>No commits to merge</strong><p>The target branch already contains this pull request head.</p></div>{/each}</section>
  {:else if tab === 'changes'}
