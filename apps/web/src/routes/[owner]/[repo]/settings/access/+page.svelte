...
<section><header><div><h3>Collaborators</h3></div><Button onclick={() => { selectedPerson = peopleOptions[0]?.value ?? ''; role = 'read'; dialog = 'person'; }}>Add collaborator</Button></header><div class="access-list">{#each collaborators as person (person.id)}<article><span class="person-link"><UserProfileLink handle={person.handle} displayName={person.displayName} avatarUrl={person.avatarUrl} size={28} showHandle /></span><div class="role-select"><Select value={person.role ?? 'read'} options={roles} ariaLabel={`Role for ${person.handle}`} onchange={(value) => updateRole('collaborators', person.id, value)} /></div><Button variant="danger-soft" size="small" icon aria-label={`Remove ${person.handle}`} onclick={() => remove('collaborators', person.id)}><Trash2 size={14} /></Button></article>{:else}<p class="empty">No direct collaborators.</p>{/each}</div></section>
<section><header><div><h3>Teams</h3></div><Button onclick={() => { selectedTeam = teamOptions[0]?.value ?? ''; role = 'read'; dialog = 'team'; }}>Add team</Button></header><div class="access-list">{#each teams as team (team.id)}<article><span class="team-avatar">{team.name.slice(0,2).toUpperCase()}</span><div><strong>{team.name}</strong><small>{team.members ?? 0} members</small></div><div class="role-select"><Select value={team.role ?? 'read'} options={roles} ariaLabel={`Role for ${team.name}`} onchange={(value) => updateRole('teams', team.id, value)} /></div><Button variant="danger-soft" size="small" icon aria-label={`Remove ${team.name}`} onclick={() => remove('teams', team.id)}><Trash2 size={14} /></Button></article>{:else}<p class="empty">No teams have explicit access.</p>{/each}</div></section>
...
<Modal open={dialog !== null} title={dialog === 'person' ? 'Add collaborator' : 'Add team'} onClose={() => (dialog = null)} actions={accessActions}><div class="dialog-form">{#if dialog === 'person'}<label><span>Person</span><Select bind:value={selectedPerson} options={peopleOptions} ariaLabel="Person" /></label>{:else}<label><span>Team</span><Select bind:value={selectedTeam} options={teamOptions} ariaLabel="Team" /></label>{/if}<label><span>Role</span><Select bind:value={role} options={roles} ariaLabel="Repository role" /></label></div></Modal>

<style>
  .security-settings{padding:0 20px;margin-bottom:16px;border-radius:14px;background:var(--surface);box-shadow:var(--shadow-surface)}
  .page-head{padding-bottom:22px;}h2{margin:0;color:var(--text-strong);font-size:22px}p{margin:6px 0 0;color:var(--text-muted);font-size:11px;line-height:1.5}section{padding:20px 0;}section>header{display:flex;align-items:center;justify-content:space-between;gap:20px}h3{margin:0;color:var(--text-strong);font-size:13px}.access-list{margin-top:13px}.access-list article{display:grid;grid-template-columns:32px minmax(0,1fr) 150px 32px;align-items:center;gap:10px;min-height:54px;}.person-link{grid-column:1/3}.team-avatar{display:grid;width:28px;height:28px;place-items:center;border-radius:8px;background:var(--brand-soft);color:var(--brand-strong);font-size:11px;font-weight:750}article strong,article small{display:block}article strong{color:var(--text-strong);font-size:11px}article small{margin-top:2px;color:var(--text-faint);font-size:11px}.role-select{width:150px}.empty{margin:0;padding:12px 0}.error{padding:9px;border-radius:8px;background:var(--danger-soft);color:var(--danger)}.dialog-form,.dialog-form label{display:grid;gap:8px}.dialog-form{gap:16px}.dialog-form label>span{color:var(--text-strong);font-size:11px;font-weight:620}
</style>
