...
<nav class="crumbs"><a href={base}>{$page.params.repo}</a><span>/</span>{#each current.split('/').filter(Boolean) as part, index}<a href="{base}/tree/{revision}/{current.split('/').slice(0,index+1).join('/')}">{part}</a><span>/</span>{/each}</nav>
<section class="tree"><header><strong>{revision}</strong><span>{current || 'Repository root'}</span></header>{#if current}<a class="row parent" href="{base}/tree/{revision}"><span><Folder size={15} />..</span><small>Parent directory</small><time></time></a>{/if}{#each entries as entry}<a class="row" href="{base}/{entry.kind === 'folder' ? 'tree' : 'blob'}/{revision}/{current ? `${current}/` : ''}{entry.name}"><span>{#if entry.kind === 'folder'}<Folder size={15} fill="currentColor" />{:else}<File size={15} />{/if}<strong>{entry.name}</strong></span><small>{entry.message}</small><time>{entry.updatedAt}</time></a>{/each}</section>
