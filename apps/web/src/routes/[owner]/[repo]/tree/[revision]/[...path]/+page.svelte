  ...
  $effect(() => {
    ...
    void (async () => {
      try {
        ...
        entries = result.entries.map((entry) => ({ name: entry.name, kind: entry.kind === 'tree' ? 'folder' : 'file', message: entry.message ?? 'History unavailable', updatedAt: entry.updatedAt ?? '' }));
      } catch {
...
<section class="tree"><header><strong>{revision}</strong><span>{current || 'Repository root'}</span></header>{#if current}<a class="row parent" href={parentHref}><span><Folder size={15} />..</span><small>Parent directory</small><time></time></a>{/if}{#each entries as entry}<a class="row" href="{base}/{entry.kind === 'folder' ? 'tree' : 'blob'}/{revisionPath}/{encodeRepositoryPath(current ? `${current}/${entry.name}` : entry.name)}"><span>{#if entry.kind === 'folder'}<Folder size={15} fill="currentColor" />{:else}<File size={15} />{/if}<strong>{entry.name}</strong></span><small>{entry.message}</small>{#if entry.updatedAt}<Time class="right" value={entry.updatedAt} />{:else}<time></time>{/if}</a>{/each}</section>
