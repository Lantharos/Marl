...
<OrganizationSettingsShell name={organizationName} {slug} avatarUrl={data.organization.avatarUrl} active="access" showSecrets={canAdminister}><header class="page-head"><h2>People and teams</h2><p>Organization membership and the access inherited by every repository.</p></header>{#if error}<p class="error">{error}</p>{/if}
  {#if isOwner && data.organization.kind !== 'personal'}<section><div class="settings-form"><label><span>Base repository role</span><Select bind:value={baseRole} options={repositoryRoles} ariaLabel="Base repository role" /></label><SettingsAction state={saveState} label="Save access" onclick={saveSettings} /></div></section>{/if}
  <section><header><div><h3>Members</h3><p>Organization roles govern teams, repositories, and invitations.</p></div>{#if canAdminister && data.organization.kind !== 'personal'}<Button onclick={() => { memberRole = 'member'; dialog = 'invite'; }}>Invite member</Button>{/if}</header><div class="list">{#each members as member (member.id)}<article><span class="person-link"><UserProfileLink handle={member.handle} displayName={member.displayName} avatarUrl={member.avatarUrl} size={28} detail={`@${member.handle}${member.email ? ` · ${member.email}` : ''}`} /></span>{#if member.role === 'owner' || !isOwner}<span class="role">{member.role}</span>{:else}<div class="role-select"><Select value={member.role} options={memberRoles} ariaLabel={`Role for ${member.handle}`} onchange={(value) => changeMember(member, value)} /></div><Button variant="danger-soft" size="small" icon aria-label={`Remove ${member.handle}`} onclick={() => removeMember(member)}><Trash2 size={14} /></Button>{/if}</article>{/each}</div></section>
  {#if invitations.length}<section><header><div><h3>Pending invitations</h3><p>Invitations expire automatically after seven days.</p></div></header><div class="list">{#each invitations as invitation (invitation.id)}<article class="invitation"><div><strong>{invitation.email}</strong><small>{invitation.role}</small></div><Button variant="danger-soft" size="small" icon aria-label={`Revoke invitation for ${invitation.email}`} onclick={() => revokeInvitation(invitation)}><Trash2 size={14} /></Button></article>{/each}</div></section>{/if}
  {#if data.organization.kind !== 'personal'}<section><header><div><h3>Teams</h3><p>Grant repository access once and keep membership centralized.</p></div>{#if canAdminister}<Button onclick={() => (dialog = 'team')}>New team</Button>{/if}</header><div class="teams">{#each teams as team (team.id)}<article><div class="team-heading"><div><strong>{team.name}</strong><span>{team.slug} · {team.members} {team.members === 1 ? 'member' : 'members'}</span></div>{#if canAdminister}<div class="buttons"><Button size="small" onclick={() => { selectedTeam = team.id; selectedUser = userOptions[0]?.value ?? ''; dialog = 'team-member'; }}>Add member</Button><Button variant="danger-soft" size="small" icon aria-label={`Delete ${team.name}`} onclick={() => { selectedTeam = team.id; dialog = 'delete-team'; }}><Trash2 size={14} /></Button></div>{/if}</div><div class="team-members">{#each teamMembers.filter((member) => member.teamId === team.id) as member (member.userId)}<div><span>{member.displayName} <small>@{member.handle}</small></span>{#if canAdminister}<Button variant="ghost" size="small" icon aria-label={`Remove ${member.handle} from ${team.name}`} onclick={() => removeTeamMember(team, member)}><Trash2 size={13} /></Button>{/if}</div>{:else}<p>No members in this team.</p>{/each}</div></article>{:else}<p>No teams yet.</p>{/each}</div></section>{/if}
  </OrganizationSettingsShell>

{#snippet dialogActions()}<Button size="small" onclick={() => (dialog = null)}>Cancel</Button><Button size="small" variant={dialog === 'delete-team' ? 'danger' : 'primary'} disabled={Boolean(busy)} onclick={() => dialog === 'invite' ? invite() : dialog === 'team' ? createTeam() : dialog === 'delete-team' ? deleteTeam() : addTeamMember()}>{dialog === 'invite' ? 'Send invitation' : dialog === 'team' ? 'Create team' : dialog === 'delete-team' ? 'Delete team' : 'Add member'}</Button>{/snippet}
...
<style>
  .page-head{padding-bottom:24px;}h2{margin:0;color:var(--text-strong);font-size:25px;letter-spacing:-.03em}.page-head p{margin:7px 0 0;color:var(--text-muted);font-size:13px;line-height:1.5}p{margin:6px 0 0;color:var(--text-muted);font-size:12px;line-height:1.5}section{padding:24px 0;}section>header{display:flex;align-items:center;justify-content:space-between;gap:18px}h3{margin:0;color:var(--text-strong);font-size:13px}.settings-form{display:grid;grid-template-columns:minmax(0,230px) auto;align-items:end;gap:12px}.settings-form label,.modal-form label{display:grid;gap:7px}.settings-form label>span,.modal-form label>span{color:var(--text-strong);font-size:12px;font-weight:620}input{height:38px;padding:0 10px;border:1px solid var(--border);border-radius:8px;outline:0;background:var(--surface);color:var(--text-strong);font-size:13px}.list{margin-top:13px}.list article{display:grid;grid-template-columns:32px minmax(0,1fr) 150px 32px;align-items:center;gap:10px;min-height:62px;}.person-link{grid-column:1/3}.list strong,.list small{display:block}.list strong{color:var(--text-strong);font-size:13px}.list small{margin-top:3px;color:var(--text-faint);font-size:11px}.list .role{grid-column:3/5;justify-self:end;color:var(--text-muted);font-size:11px}.role-select{width:150px}.list article.invitation{grid-template-columns:1fr 34px}.buttons{display:flex;gap:7px}.teams{display:grid;gap:8px;margin-top:14px}.teams>article{padding:14px;border-radius:8px;background:var(--surface)}.team-heading{display:flex;align-items:center;justify-content:space-between;gap:12px}.teams strong,.teams span{display:block}.teams strong{color:var(--text-strong);font-size:13px}.teams span{margin-top:4px;color:var(--text-faint);font-size:11px}.team-members{display:grid;margin-top:11px;}.team-members>div{display:flex;min-height:40px;align-items:center;justify-content:space-between}.team-members small{color:var(--text-faint)}.modal-form{display:grid;gap:15px}.error{display:flex;align-items:center;gap:7px;padding:10px;border-radius:8px;background:var(--danger-soft);color:var(--danger);font-size:12px}@media(max-width:760px){.settings-form{grid-template-columns:1fr}.teams{grid-template-columns:1fr}}
</style>
