  ...
  <section class="list" aria-label="Pull requests">
    {#each items as pull}
      <article class="row">
        ...
        <span class="main"><a class="title-link" href="/{pull.repository.owner}/{pull.repository.name}/pulls/{pull.number}">{pull.title}</a><small>{pull.repository.owner}/{pull.repository.name} #{pull.number} opened by <a class="author-link" href="/{pull.author}">{pull.authorDisplayName}</a> · <Time value={pull.updatedAt} /></small><code>{pull.sourceBranch}<ArrowRight size={11} />{pull.targetBranch}</code></span>
...
<style>
  .page{width:min(1080px,calc(100% - 64px));margin:0 auto;padding:48px 0 72px}.empty{padding:70px 20px;color:var(--text-muted);text-align:center}.empty strong{display:block;margin-top:10px;color:var(--text-strong);font-size:15px}.empty p{margin:7px auto 0;max-width:420px;font-size:12px}.empty a{display:inline-flex;margin-top:16px;color:var(--brand-strong);font-size:12px;text-decoration:none}.row{position:relative;display:grid;grid-template-columns:36px minmax(0,1fr) 145px 72px;align-items:center;gap:12px;min-height:84px;padding:12px 5px;border-bottom:1px solid var(--border-subtle);color:inherit}.row:hover{background:var(--surface-hover)}.state{display:grid;width:32px;height:32px;place-items:center;border-radius:8px;background:var(--brand-soft);color:var(--brand)}.state.blocked{background:var(--danger-soft);color:var(--danger)}.state.ready{background:var(--success-soft);color:var(--success)}.state.merged{background:color-mix(in srgb,#8b5cf6 18%,transparent);color:#a78bfa}.main{min-width:0}.main .title-link,.main small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.main .title-link{color:var(--text-strong);font-size:14px;font-weight:640;text-decoration:none}.main .title-link::after{position:absolute;z-index:0;inset:0;content:''}.main small{margin-top:5px;color:var(--text-muted);font-size:12px}.main .author-link{position:relative;z-index:1;color:var(--text-strong);text-decoration:none}.main .author-link:hover{color:var(--brand)}.main code{display:flex;align-items:center;gap:5px;margin-top:5px;color:var(--text-muted);font-size:11px}.review,.checks{display:inline-flex;align-items:center;gap:5px;color:var(--text-muted);font-size:12px;font-weight:580}.checks{color:var(--success)}.checks.failed{color:var(--danger)}.checks.empty-checks{color:var(--text-muted)}.load-more{display:block;height:36px;margin:18px auto 0;padding:0 14px;border:1px solid var(--border);border-radius:6px;background:var(--surface);color:var(--text);cursor:pointer;font-size:12px}@media(max-width:760px){.page{width:calc(100% - 28px);padding-top:28px}.row{grid-template-columns:36px minmax(0,1fr) 44px}.review{display:none}}
