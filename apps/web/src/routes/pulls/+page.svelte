  ...
  <section class="list" aria-label="Pull requests">
    {#each filteredItems as pull}
      <a class="row" href="/{pull.repository.owner}/{pull.repository.name}/pulls/{pull.number}">
        <span class:blocked={pull.state === 'blocked'} class:ready={pull.state === 'mergeable'} class:merged={pull.state === 'merged'} class="state">{#if pull.state === 'merged'}<GitMerge size={17} />{:else}<GitPullRequest size={17} />{/if}</span>
