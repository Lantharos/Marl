  ...
  <section class="list" aria-label="Pull requests">
    {#each items as pull (pull.id)}
      <article class="row">
        ...
        <span class="main"><a class="title-link" href="/{pull.repository.owner}/{pull.repository.name}/pulls/{pull.number}">{pull.title}</a><small>{pull.repository.owner}/{pull.repository.name} !{pull.number} opened by <a class="author-link" href="/{pull.author}">{pull.authorDisplayName}</a> · <Time value={pull.updatedAt} /></small><span class="details"><code>{pull.sourceBranch}<ArrowRight size={11} />{pull.targetBranch}</code><span class="checks" class:failed={pull.checkSummary.failed > 0} class:empty-checks={pull.checkSummary.total === 0} class:running={pull.checkSummary.running > 0}>{#if pull.checkSummary.failed}<CircleAlert size={12} />{pull.checkSummary.failed} failed{:else if pull.checkSummary.running}<CircleDot size={12} />{pull.checkSummary.running} running{:else if pull.checkSummary.total === 0}<CircleDot size={12} />No checks{:else}<CircleCheck size={12} />{pull.checkSummary.passed}/{pull.checkSummary.total} passed{/if}</span>{#each pull.labels.slice(0, 3) as label (label.id)}<span class="label" style:--label-color={label.color}>{label.name}</span>{/each}{#if pull.labels.length > 3}<span class="more-labels">+{pull.labels.length - 3}</span>{/if}</span></span>
