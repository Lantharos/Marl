...
try {
  ...
  const created = await client.request<{ repository: { id: string } }>('/api/v1/repositories', {
    method: 'POST',
    body: JSON.stringify({ owner: qualificationOwner, name: repositoryName, description: 'Isolated Marl qualification repository', visibility: 'private' })
  });
  ...
  await run(['git', 'config', 'gpg.ssh.program', 'ssh-keygen'], { cwd: source });
  ...
  const workflows = await client.waitFor(
    () => client.request<{ workflows: Array<{ id: string; status: string }> }>(`/api/v1/repositories/${qualificationOwner}/${repositoryName}/workflows`),
  ...
  const enrollment = await client.request<{ enrollment: { token: string } }>('/api/v1/runner-enrollments', {
    method: 'POST',
    body: JSON.stringify({ organization: qualificationOwner, expiresMinutes: 15 })
  });
  await run([
  ...
  const runDetail = await client.request<{ run: { jobsDetail: Array<{ id: string; state: string; artifacts: Array<{ id: string; name: string }> }> } }>(`/api/v1/repositories/${qualificationOwner}/${repositoryName}/runs/${completed.number}`);
  ...
  for (const method of ['merge', 'squash', 'rebase'] as const) {
    ...
    const detail = await client.request<{ pullRequest: { events: Array<{ kind: string }> } }>(`/api/v1/repositories/${qualificationOwner}/${repositoryName}/pulls/${pull.pullRequest.number}`);
  ...
  console.log('\nMarl qualification passed. Git history, SSH commit signing, PR publication, runner execution, supersession, restart recovery, and strict clone integrity are healthy.');
...
function startApi() {
  return new ManagedService([
    ...
    '--var', `GIT_PUBLIC_URL:${gitUrl}`, '--var', `GIT_SSH_PUBLIC_URL:${sshUrl}`, '--var', `GIT_GATEWAY_TOKEN:${gatewayToken}`, '--var', `PUBLIC_URL:${apiUrl}`,
...
function startGit() {
  return new ManagedService([executable('git-gateway')], {
    cwd: root,
    env: { MARL_GIT_ROOT: repositories, MARL_API_URL: apiUrl, MARL_GIT_LISTEN: `127.0.0.1:${gitPort}`, MARL_SSH_LISTEN: `127.0.0.1:${sshPort}`, MARL_GIT_LOCAL: '1', MARL_GIT_GATEWAY_TOKEN: gatewayToken }
  });
}
