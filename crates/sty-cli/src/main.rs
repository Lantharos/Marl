...
use sty_core::Repository;

#[derive(Parser)]
#[command(
    name = "sty",
    version,
    about = "Work with Sty repositories and runners"
)]
struct Cli {
...
async fn runner(command: RunnerCommand) -> Result<()> {
    match command {
        RunnerCommand::Register {
            ...
            let config_path = config.unwrap_or(sty_runner::config::default_config_path()?);
            let work_dir = work_dir.unwrap_or(sty_runner::config::default_work_dir()?);
            let registered = sty_runner::register(sty_runner::RegisterOptions {
                url: &url,
            ...
            println!(
                "Start: sty runner run --config \"{}\"",
                config_path.display()
            );
            Ok(())
        }
        RunnerCommand::Run { config, once } => {
            let path = config.unwrap_or(sty_runner::config::default_config_path()?);
            sty_runner::executor::run(sty_runner::config::load(&path)?, once).await
        }
        RunnerCommand::Service { command } => match command {
            ServiceCommand::Install { config } => {
                let path = config.unwrap_or(sty_runner::config::default_config_path()?);
                sty_runner::config::load(&path)?;
                sty_runner::service::install(&path)
            }
            ServiceCommand::Uninstall => sty_runner::service::uninstall(),
            ServiceCommand::Status => sty_runner::service::status(),
        },
    }
}
...
fn machine_name() -> String {
    std::env::var("COMPUTERNAME")
        ...
        .unwrap_or_else(|_| "sty-runner".to_owned())
        .to_lowercase()
        .replace(
            |character: char| {
