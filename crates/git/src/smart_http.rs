        ...
        Err(status) => {
            ...
            if status == StatusCode::UNAUTHORIZED {
                response.headers_mut().insert(
                    "www-authenticate",
                    HeaderValue::from_static("Basic realm=\"Marl\", charset=\"UTF-8\""),
                );
            }
            return Ok(response);
        }
...
mod tests {
    ...
    fn parses_smart_git_paths() {
        let value = parse_git_path(
            "/lantharos/marl.git/info/refs",
            Some("service=git-receive-pack"),
        )
        .unwrap();
        assert_eq!(value.owner, "lantharos");
        assert_eq!(value.repository, "marl");
        assert_eq!(value.service, "git-receive-pack");
    }
    #[test]
    fn rejects_traversal_and_non_git_routes() {
        assert!(parse_git_path("/../marl.git/info/refs", None).is_none());
        assert!(parse_git_path("/lantharos/marl/info/refs", None).is_none());
    }
    #[test]
