...
pub fn default_config_path() -> Result<PathBuf> {
    if let Some(path) = std::env::var_os("MARL_RUNNER_CONFIG") {
    ...
    Ok(if cfg!(windows) {
        PathBuf::from(root).join("Marl").join("runner.json")
    } else {
        PathBuf::from(root).join(".config/marl/runner.json")
    })
}
...
pub fn default_work_dir() -> Result<PathBuf> {
    ...
    Ok(if cfg!(windows) {
        PathBuf::from(root).join("Marl").join("work")
    } else {
        PathBuf::from(root).join(".local/share/marl/work")
    })
}
