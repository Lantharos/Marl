...
export const pigCommandGroups: CommandGroup[] = [
	{
		...
		rows: [
			...
			{ command: 'pig pack 4..10 "message"', description: 'Pack older saves while replaying the newer saves on top.' },
			{ command: 'pig unpack', description: 'Undo the latest pack operation and restore the previous local stack.' },
	...
	},
	{
		...
		rows: [
			...
			{ command: 'sty ci runner new <tenant>/<project> linux-builder', description: 'Create a project-scoped self-hosted CI runner token.' },
			{ command: 'pig ci runner setup --labels linux,rust', description: 'Validate runner credentials and print copy-paste runner setup commands.' },
			{ command: 'pig ci run --labels linux,rust', description: 'Run a self-hosted runner and advertise labels for compatible queued jobs.' },
			{ command: 'pig ci detect', description: 'Detect runnable jobs, matrix variants, path filters, env, and runner labels from existing .github/workflows files or local project files.' },
			{ command: 'pig ci detect --push', description: 'Merge detected jobs, filters, env, and labels into the configured project CI settings.' },
			{ command: 'pig ci detect --push --replace', description: 'Replace project CI commands with the detected jobs.' },
			{ command: 'pig ci jobs', description: 'List recent remote CI jobs for the configured project.' },
			{ command: 'pig ci jobs --workspace feature-auth', description: 'List recent remote CI jobs for one workspace.' },
			{ command: 'pig ci logs <job-id>', description: 'Print uploaded logs for one remote CI job.' },
			{ command: 'pig ci cancel <job-id>', description: 'Cancel a queued or running remote CI job.' },
			{ command: 'pig ci rerun <job-id>', description: 'Queue a fresh attempt for one remote CI job.' },
			{ command: 'pig ci artifacts <job-id>', description: 'List artifacts uploaded by one remote CI job.' },
			{ command: 'pig ci artifacts <job-id> --download <artifact-id>', description: 'Download one remote CI artifact.' },
			{ command: 'pig ci secrets list', description: 'List project CI secret keys.' },
			{ command: 'pig ci secrets set NPM_TOKEN --value-stdin', description: 'Set a project CI secret without printing the value in shell history.' },
			{ command: 'pig ci secrets delete NPM_TOKEN', description: 'Delete a project CI secret.' },
			{ command: 'pig ci doctor', description: 'Check local detection, remote CI settings, auth, job access, secrets access, and runner token setup.' },
