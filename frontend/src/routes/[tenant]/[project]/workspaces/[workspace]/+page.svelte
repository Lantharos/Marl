<script lang="ts">
	...
	async function load() {
		...
		try {
			...
			const firstFile = detail.files.entries.find((e) => e.entry_type === 'blob');
			if (firstFile) {
				file = await getProjectFile(tenant, project, firstFile.path, workspaceName);
			}
		} catch (e) {
		...
		<div class="grid gap-5 xl:grid-cols-[1fr_300px]">
			<div class="rounded border border-[#2a2a28] bg-[#141412] p-4">
				...
				<div class="flex gap-0" style="height: calc(100vh - 220px);">
					<div class="w-[260px] shrink-0 border-r border-[#2a2a28] pr-3">
