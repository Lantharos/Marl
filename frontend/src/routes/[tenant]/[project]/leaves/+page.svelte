<script lang="ts">
	...
	async function load(signal?: AbortSignal, query = '') {
		...
		error = '';
		try {
			const result = await listProjectLeavesPage(tenant, project, {
				page: 1,
				perPage: chunkSize,
				q: query || undefined,
				signal
	...
	async function loadMore() {
		...
		loadingMore = true;
		try {
			const result = await listProjectLeavesPage(requestTenant, requestProject, {
				page: nextPage,
				perPage: chunkSize,
				q: requestQuery || undefined
			});
			if (requestTenant !== tenant || requestProject !== project || requestQuery !== searchQuery) return;
			leaves = appendUnique(leaves, result.items);
			total = result.total;
			nextPage = result.next;
		} catch (e) {
			error = e instanceof Error ? e.message : 'Failed to load leaves';
	...
	function appendUnique(current: Leaf[], incoming: Leaf[]) {
		const seen = new Set(current.map((leaf) => leaf.id));
		return [
			...current,
			...incoming.filter((leaf) => {
	...
	function visibilityIcon(leaf: Leaf) {
		if (leaf.visibility === 'public') return Globe2;
		if (leaf.visibility === 'private') return LockKeyhole;
		return Users;
	}
...
<div class="mx-auto max-w-6xl">
	<div class="mb-5 flex flex-wrap items-center gap-3">
		...
		{#if canWrite}
			<a class="inline-flex h-9 items-center gap-1 bg-[#eae9e4] px-3 text-sm text-[#0f0f0d] hover:bg-[#d8d3c5]" href={`/${tenant}/${project}/leaves/new`}>
	...
	{:else}
		<div class="border border-[#2a2a28] bg-[#0f0f0d]">
			<div class="flex min-h-11 items-center justify-between border-b border-[#2a2a28] bg-[#141412] px-4 text-sm text-[#a09d94]">
				<div>
					<span class="text-[#eae9e4]">All</span>
					<span class="ml-1 text-xs text-[#6f6b5f]">{total}</span>
				</div>
				<span>Updated</span>
			</div>
			{#each leaves as leaf (leaf.id)}
				{@const Icon = visibilityIcon(leaf)}
				<a class="group grid min-h-16 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b border-[#252522] px-4 py-3 last:border-b-0 hover:bg-[#1a1a18]" href={leaf.href}>
					...
					<div class="min-w-0">
						<div class="flex min-w-0 items-center gap-2">
							...
							{#if leaf.pinned}
								<Pin class="h-3.5 w-3.5 shrink-0 text-[#d9a66c]" />
							{/if}
						</div>
						<div class="mt-1 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 text-xs text-[#8c887e]">
							<span class="inline-flex items-center gap-1"><Icon class="h-3 w-3" /> {leaf.visibility}</span>
							...
							{#each leaf.tags.slice(0, 4) as tag (tag)}
