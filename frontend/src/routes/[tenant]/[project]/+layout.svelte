<script lang="ts">
	...
	const overview = $derived((($page.data as { overview?: ProjectOverview | null }).overview ?? null));
	const layoutChrome = $derived(
		($page.data as { projectChrome?: { settings: ProjectSettings | null; stats: ProjectStats | null } }).projectChrome ?? null
	);
	const settings = $derived(publicSettings ?? overview?.settings ?? layoutChrome?.settings ?? null);
	const stats = $derived<ProjectStats | null>(publicStats ?? overview?.stats ?? layoutChrome?.stats ?? null);
	...
	function active(href: string) {
		return href === `/${tenant}/${project}` ? currentPath === href : currentPath.startsWith(href);
	}

	async function loadPublicChrome(tenant: string, project: string, signal: AbortSignal) {
		try {
		...
		<div class="px-32 md:px-48 lg:px-64 xl:px-80">
			<div class="flex items-center gap-4 py-2.5">
				...
				<div class="flex min-w-0 flex-1 items-center gap-0.5">
					<a class="flex min-w-0 items-center gap-1 rounded px-2 py-1 text-[14px] font-medium leading-5 text-[#a09d94] hover:bg-[#1e1e1c] hover:text-[#d9a66c]" href="/{tenant}"><span class="truncate">{tenant}</span></a>
					<span class="text-[#5c5c5a]">/</span>
					<a class="flex min-w-0 items-center gap-1 rounded px-2 py-1 text-[14px] font-medium leading-5 text-[#eae9e4] hover:bg-[#1e1e1c] hover:text-[#d9a66c]" href="/{tenant}/{project}"><span class="truncate">{project}</span></a>
				</div>
			...
			<nav class="flex gap-1 overflow-x-auto">
				{#each tabs() as tab}
					...
					{:else}
						{@const href = tab.id ? `/${tenant}/${project}/${tab.id}` : `/${tenant}/${project}`}
						{@const count = projectTabCount(stats, tab.id)}
						<a
							{href}
							class="inline-flex items-center gap-1.5 border-b-2 px-3 py-2 text-sm font-medium {active(href) ? 'border-[#d9a66c] text-[#f0eee4]' : 'border-transparent text-[#8c887e] hover:text-[#d9a66c]'}"
						>
							{tab.label}
							{#if count !== null}
