...
static DEVELOPER_SCHEMA_READY: AtomicBool = AtomicBool::new(false);
...
pub async fn ensure_developer_schema(db: &Database) -> Result<()> {
    if DEVELOPER_SCHEMA_READY.load(Ordering::Relaxed) {
        return Ok(());
    }
    ...
    DEVELOPER_SCHEMA_READY.store(true, Ordering::Relaxed);
    Ok(())
}
