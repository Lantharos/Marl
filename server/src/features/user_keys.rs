...
static USER_KEYS_SCHEMA_READY: AtomicBool = AtomicBool::new(false);
...
async fn ensure_user_keys_schema(db: &Database) -> Result<()> {
    if USER_KEYS_SCHEMA_READY.load(Ordering::Relaxed) {
        return Ok(());
    }
    db.prepare(
    ...
    .run()
    .await?;
    USER_KEYS_SCHEMA_READY.store(true, Ordering::Relaxed);
    Ok(())
}
