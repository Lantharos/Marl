...
pub async fn followed_activity(
    ...
    let (history_items, issue_items, release_items) = try_join3(
        followed_history_activity(db, principal, limit),
        followed_issue_activity(db, principal, limit),
        followed_release_activity(db, principal, limit),
    )
    .await?;
...
pub async fn project_activity(
    ...
    ensure_collaboration_schema(db).await?;
    let (history_items, issue_items, release_items) = try_join3(
        project_history_activity(db, principal, limit),
        project_issue_activity(db, principal, limit),
        project_release_activity(db, principal, limit),
    )
    .await?;
    let mut items = history_items;
    items.extend(issue_items);
