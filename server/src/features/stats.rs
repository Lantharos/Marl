...
pub async fn project_stats(db: &Database, tenant: &str, project: &str) -> Result<ProjectStats> {
...
pub async fn recompute_project_stats(db: &Database, tenant: &str, project: &str) -> Result<()> {
    let updated_at = now_rfc3339();
    db.prepare(
        "INSERT INTO project_stats (
            tenant, project, workspace_count, open_issue_count, ready_count,
            release_count, history_count, leaf_count, updated_at
         )
         VALUES (
            ?1,
            ?2,
            ...
            (SELECT COUNT(*) FROM leaves WHERE tenant = ?1 AND project = ?2),
            ?3
         )
...
async fn select_project_stats(
    ...
    struct Row {
        ...
        release_count: f64,
        history_count: f64,
        leaf_count: Option<f64>,
    }
    let row: Option<Row> = db
        .prepare(
            "SELECT workspace_count, open_issue_count, ready_count, release_count, history_count, leaf_count
             FROM project_stats
    ...
    Ok(row.map(|row| ProjectStats {
        ...
        leaf_count: row.leaf_count.unwrap_or(0.0) as u64,
    }))
}
