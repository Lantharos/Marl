...
static GOVERNANCE_SCHEMA_READY: AtomicBool = AtomicBool::new(false);
...
pub async fn ensure_governance_schema(db: &Database) -> Result<()> {
    if GOVERNANCE_SCHEMA_READY.load(Ordering::Relaxed) {
        return Ok(());
    }
    db.prepare(
    ...
    .run()
    .await?;
    GOVERNANCE_SCHEMA_READY.store(true, Ordering::Relaxed);
    Ok(())
}
