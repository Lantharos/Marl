...
pub async fn enqueue_ci_jobs_for_head(
        ...
        .take(ci.max_jobs_per_head.clamp(1, 100) as usize)
    {
        ...
        if !ci_command_matches_paths(command, changed_paths) {
...
async fn mark_ci_job_skipped_for_head(
    ...
    upsert_workspace_check(
        ...
        "completed",
        Some("skipped"),
        Some("No changed paths matched this CI command."),
        None,
    )
    .await?;
    Ok(())
...
fn ci_command_matches_workspace(command: &CiCommand, workspace: &str) -> bool {
    command.workspaces.is_empty()
        || command
            .workspaces
            .iter()
...
pub async fn claim_next_ci_job(
    ...
    ensure_ci_schema(db).await?;
    CiQueueScheduler { db, runner, ci }.claim_next().await
}

struct CiQueueScheduler<'a> {
    db: &'a Database,
    runner: &'a CiRunner,
    ci: &'a ProjectCiSettings,
}

impl CiQueueScheduler<'_> {
    async fn claim_next(&self) -> Result<Option<CiJob>> {
        reclaim_expired_ci_jobs(self.db, &self.runner.tenant, &self.runner.project).await?;
        if !self.has_capacity().await? {
            return Ok(None);
        }
        for row in self.queued_candidates().await? {
            if let Some(reason) = self.incompatibility_reason(&row) {
                self.mark_waiting(&row, &reason).await?;
                continue;
            }
            if let Some(job) = self.claim_row(&row).await? {
                return Ok(Some(job));
            }
        }
        return Ok(None);
    ...
    async fn has_capacity(&self) -> Result<bool> {
        if active_project_job_count(self.db, &self.runner.tenant, &self.runner.project).await?
            >= self.ci.max_concurrent_jobs.clamp(1, 100)
        {
            return Ok(false);
        }
        Ok(active_runner_job_count(self.db, &self.runner.id).await?
            < self.runner.concurrency.clamp(1, 32))
    }
    ...
    fn incompatibility_reason(&self, row: &CiJobRow) -> Option<String> {
        let command = ci_command_for_job(row, self.ci)?;
        let missing = missing_runner_labels(&command.labels, &self.runner.labels);
        if missing.is_empty() {
            return None;
        }
        Some(format!(
            "Waiting for a runner with labels: {}.",
            missing.join(", ")
        ))
    }

    async fn mark_waiting(&self, row: &CiJobRow, reason: &str) -> Result<()> {
        upsert_workspace_check(
            self.db,
            &row.tenant,
            &row.project,
    ...
    async fn claim_row(&self, row: &CiJobRow) -> Result<Option<CiJob>> {
        let now = now_rfc3339();
        let lease_expires_at = rfc3339_seconds_from_now(ci_lease_seconds(
            row.timeout_seconds as u32,
            self.ci.lease_grace_seconds,
        ));
        let max_attempts = self.ci.max_attempts.clamp(1, 10);
        let max_project_jobs = self.ci.max_concurrent_jobs.clamp(1, 100);
        let max_runner_jobs = self.runner.concurrency.clamp(1, 32);
        let result = self
            .db
            .prepare(
                "UPDATE ci_jobs
            ...
            .bind(&[
                js_str(&self.runner.id),
                js_str(&now),
                js_str(&lease_expires_at),
                wasm_bindgen::JsValue::from_f64(max_attempts as f64),
                js_str(&row.id),
                js_str(&self.runner.tenant),
                js_str(&self.runner.project),
        ...
        .await?;
        ci_job(self.db, &row.tenant, &row.project, &row.id).await
    }
}

fn missing_runner_labels<'a>(required: &'a [String], labels: &[String]) -> Vec<&'a str> {
    required
        .iter()
        .filter(|label| !labels.iter().any(|runner_label| runner_label == *label))
        .map(String::as_str)
        .collect()
}
...
pub async fn cancel_ci_job(
    ...
    ensure_ci_schema(db).await?;
    let Some(existing) = ci_job(db, tenant, project, job_id).await? else {
        return Ok(None);
    };
    ...
    let result = db
        .prepare(
            "UPDATE ci_jobs
             SET status = 'completed', conclusion = 'canceled', summary = ?1, runner_id = NULL, lease_expires_at = NULL, completed_at = ?2, updated_at = ?2
             WHERE id = ?3 AND tenant = ?4 AND project = ?5 AND status IN ('queued', 'in_progress')",
        )
        .bind(&[
            js_str(summary),
            js_str(&now),
        ...
        .await?;
    if result.meta()?.and_then(|meta| meta.changes).unwrap_or(0) == 0 {
        return ci_job(db, tenant, project, job_id).await;
    }
    upsert_workspace_check(
        db,
        tenant,
    ...
    .await?;
    ci_job(db, tenant, project, job_id).await
}

pub async fn rerun_ci_job(
    db: &Database,
    tenant: &str,
    project: &str,
    job_id: &str,
