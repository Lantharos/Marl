...
static PROFILE_SCHEMA_READY: AtomicBool = AtomicBool::new(false);
...
async fn ensure_profile_schema(db: &Database) -> Result<()> {
    if PROFILE_SCHEMA_READY.load(Ordering::Relaxed) {
        return Ok(());
    }
    db.prepare(
    ...
    .run()
    .await?;
    PROFILE_SCHEMA_READY.store(true, Ordering::Relaxed);
    Ok(())
}
