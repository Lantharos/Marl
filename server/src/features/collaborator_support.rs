...
static COLLABORATION_SCHEMA_READY: AtomicBool = AtomicBool::new(false);
...
pub async fn ensure_collaboration_schema(db: &Database) -> Result<()> {
    if COLLABORATION_SCHEMA_READY.load(Ordering::Relaxed) {
        return Ok(());
    }
    db.prepare(
    ...
    .run()
    .await?;
    COLLABORATION_SCHEMA_READY.store(true, Ordering::Relaxed);
    Ok(())
}
