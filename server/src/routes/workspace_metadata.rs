...
use crate::routes::objects::{
    check_project_write_capability, check_workspace_write_capability, require_auth,
};
...
pub(crate) async fn update_workspace_metadata(
    ...
    let visibility = if let Some(value) = body.get("visibility") {
        ...
        let next = features::normalize_workspace_visibility(value)?;
        if workspace == "main" && next != features::WORKSPACE_VISIBILITY_PUBLIC {
            return json_error(400, "main workspace must stay public within the project");
        }
        if next != state.visibility {
            if !features::workspace_can_manage_visibility(
                &database, &tenant, &project, &user, &workspace,
            )
            .await?
            {
                return json_error(403, "workspace visibility access denied");
            }
            features::set_workspace_visibility(&database, &tenant, &project, &workspace, &next)
                .await?;
        }
        next
    } else {
    ...
    features::set_workspace_metadata(
        &database,
        &tenant,
        &project,
        &workspace,
