...
use crate::features;
use crate::routes::leaves::visible_project_leaves;
use crate::routes::objects::{
    check_project_capability, check_project_read_capability, check_project_write_capability,
    check_workspace_read_capability, optional_auth, require_auth,
};
use crate::routes::protocol_profiles::profile_json;
use crate::support::{
...
pub async fn update_protocol_comment(
    ...
    if features::project_is_archived(&database, &tenant, &project).await? {
...
pub async fn search_project(
    ...
    for issue in features::list_issues(&database, &tenant, &project).await? {
    ...
    for entry in features::project_history(&database, &tenant, &project).await? {
...
pub async fn list_reactions(
    ...
    let reactions = features::list_reactions(
        &database,
        &tenant,
        &project,
        &target_kind,
...
pub async fn add_reaction(
    ...
    let reactions = features::add_reaction(
        &database,
        &tenant,
        &project,
        &target_kind,
...
pub async fn verify_snapshot(
    ...
    check_project_read_capability(&database, &tenant, &project, user.as_deref(), "main:read")
        .await?;
    let result = crate::routes::account_keys::verify_snapshot_id(
        &database, &ctx.env, &tenant, &project, &id,
    )
    .await?;
...
pub async fn verify_all_snapshots(
    ...
    for id in features::object_ids_by_kind(&database, &tenant, &project, "snapshot").await? {
        snapshots.push(
            crate::routes::account_keys::verify_snapshot_id(
                &database, &ctx.env, &tenant, &project, &id,
            )
            .await?,
        );
...
pub async fn delete_protocol_item(
    ...
    database
        ...
        .await?;
    if kind.as_deref() == Some("comment") {
        features::delete_reactions_for_target(&database, &tenant, &project, "comment", &id).await?;
    }
...
async fn enrich_protocol_comment_profiles(
    ...
    for item in items {
        ...
        if !profiles.contains_key(&author) {
            profiles.insert(
                author.clone(),
                features::user_profile(database, &author).await?,
            );
        }
...
async fn protocol_comment_visible(
    ...
    features::workspace_can_read(database, tenant, project, user, workspace).await
}
