...
pub(crate) async fn upload_objects(
    ...
    for (id, kind, bytes) in objects {
        ...
        let size = bytes.len();
        put_bytes(&features, &object_key(&tenant, &project, &id), bytes).await?;
        features::record_object(&database, &tenant, &project, &id, &kind, size).await?;
        stored += 1;
    }
...
pub(crate) async fn object_path_closure(
    ...
    let workspace_head = features::head(&database, &tenant, &project, &workspace).await?;
...
pub(crate) async fn put_object(
    ...
    .await?;
    if features::object_kind(&database, &tenant, &project, &id)
        .await?
        .is_some()
    {
...
pub(crate) async fn get_object(
    ...
    let bytes = body.bytes().await?;
    let kind = features::object_kind(&database, &tenant, &project, &id)
        .await?
...
pub(crate) fn object_digest_for_kind(bytes: &[u8], kind: &str) -> Result<String> {
    if kind != "snapshot" {
...
impl ObjectPathClosure {
    fn add_object(&mut self, id: String, kind: String) -> Result<()> {
        ...
        if self.objects.len() >= PATH_CLOSURE_OBJECT_LIMIT {
            return Err(Error::RustError(
                "path object closure limit exceeded".to_string(),
            ));
        }
...
async fn collect_path_closure(
    ...
    if path.is_empty() {
        collect_tree_subtree(features, tenant, project, root_tree, "", closure).await?;
        return Ok(true);
    }
    ...
    for (index, part) in parts.iter().enumerate() {
        ...
        if index + 1 == parts.len() {
            if entry.entry_type == "blob" {
                ...
                collect_tree_subtree(features, tenant, project, &entry.id, &current_path, closure)
                    .await?;
            }
            return Ok(true);
        }
...
pub(crate) async fn require_web_auth(
    ...
    let token = bearer_token_from_request(req)?;
    let database = db(ctx)?;
    let principal = features::principal_for_token(&database, &token)
        .await?
        .ok_or_else(|| Error::RustError("invalid bearer token".to_string()))?;
    match features::token_kind(&database, &token).await?.as_deref() {
...
pub(crate) async fn check_project_access(
    ...
    if let Some(u) = user {
        if features::project_access(db, tenant, project, u).await?
            || matches!(
                features::project_visibility(db, tenant, project).await?,
                Some(v) if v == "public"
            )
        {
            Ok(())
        ...
        match features::project_visibility(db, tenant, project).await? {
...
pub(crate) async fn check_project_role(
    ...
    if features::project_role_allows(db, tenant, project, user, minimum).await? {
        return Ok(());
    }
    Err(Error::RustError(format!("project {minimum} access denied")))
}
...
pub(crate) async fn check_project_read_capability(
    ...
    if let Some(user) = user.filter(|value| value.starts_with("api-key:")) {
        ...
        if features::project_api_key_allows(db, tenant, project, user, capability)
            .await?
            .unwrap_or(false)
        {
            return Ok(());
    ...
    if let Some(user) = user.filter(|value| value.starts_with("ci-runner:")) {
        ensure_project_target(db, tenant, project).await?;
        if features::ci_runner_allows(db, tenant, project, user, capability)
            .await?
            .unwrap_or(false)
        {
            return Ok(());
...
pub(crate) async fn check_project_write_role(
    ...
    check_project_role(db, tenant, project, user, minimum).await?;
    if features::project_is_archived(db, tenant, project).await? {
        return Err(Error::RustError(
...
pub(crate) async fn check_workspace_read_capability(
    ...
    if features::workspace_can_read(db, tenant, project, user, workspace).await? {
        return Ok(());
    }
...
pub(crate) async fn check_workspace_write_capability(
    ...
    let exists = features::workspace_exists(db, tenant, project, workspace).await?;
    let capability = if workspace == "main" {
        "main:write"
    } else if exists {
        "workspaces:write"
    ...
    if !exists || features::workspace_can_write(db, tenant, project, user, workspace).await? {
        return Ok(());
    }
    Err(Error::RustError("workspace access denied".to_string()))
}
