...
use crate::features;
use crate::routes::objects::{check_project_write_capability, require_auth};
...
pub(crate) async fn fork_project(
    ...
    let visibility =
        features::project_visibility(&database, &body.source_tenant, &body.source_project)
            .await?
    ...
    let source = features::get_project(&database, &body.source_tenant, &body.source_project)
        .await?
        .ok_or_else(|| Error::RustError("source project not found".to_string()))?;
    let workspace = contribution_workspace(&body, &user)?;
    features::create_fork_project(
        &database,
        &body.source_tenant,
        &body.source_project,
        &body.target_tenant,
...
pub(crate) async fn send_work(
    ...
    .await?;
    let Some(fork) = features::project_fork(&database, &tenant, &project).await? else {
        return json_error(
            409,
            "sendwork only works from a linked contribution fork; use normal workspace review for this project",
        );
    };
    let head = features::head(&database, &tenant, &project, &body.workspace)
        .await?
    ...
    .await?;
    let principal = sty_protocol::TokenPrincipal { user: user.clone() };
    features::publish_fork_workspace(
        &database,
        &fork,
        body.title.trim(),
        body.message.trim(),
    ...
    .await?;
    let source = features::get_project(&database, &fork.source_tenant, &fork.source_project)
        .await?
        .ok_or_else(|| Error::RustError("source project not found".to_string()))?;
...
async fn copy_project_objects(
    ...
    for object in features::project_objects(database, source_tenant, source_project).await? {
        if features::object_kind(database, target_tenant, target_project, &object.id)
            .await?
            .is_none()
        {
            let bytes = r2_bytes(
