use super::prelude::*;
pub(crate) async fn project_issues(
    req: Request,
    ...
    check_project_read_capability(&database, &tenant, &project, user.as_deref(), "issues:read")
        .await?;
    let mut issues = features::list_issues(&database, &tenant, &project).await?;
    let url = req.url()?;
    let state = url
        .query_pairs()
    ...
    let label = url
        .query_pairs()
        .find_map(|(k, v)| (k == "label").then(|| v.to_string()));
    let assignee = url
        .query_pairs()
        .find_map(|(k, v)| (k == "assignee").then(|| v.to_string()));
    let issue_type = url
        .query_pairs()
...
pub(crate) async fn get_issue(
    ...
    check_project_read_capability(&database, &tenant, &project, user.as_deref(), "issues:read")
        .await?;
    let issue = features::list_issues(&database, &tenant, &project)
        .await?
        .into_iter()
...
pub(crate) async fn update_issue(
    ...
    let before = features::list_issues(&database, &tenant, &project)
        .await?
        .into_iter()
        .find(|issue| issue.id == issue_id || issue.number.to_string() == issue_id)
    ...
    .await?;
    record_issue_metadata_activity(
        &database, &tenant, &project, &issue.id, &user, &before, &issue,
    )
    .await?;
    let _ = crate::webhooks::emit_project_event(
        &ctx,
...
pub(crate) async fn set_issue_state(
    mut req: Request,
    ctx: crate::request_context::AppRouteContext,
    state: &str,
) -> Result<Response> {
    ...
    let issue =
        features::update_issue_status(&database, &tenant, &project, &issue_id, state, state_reason)
            .await?;
...
pub(crate) async fn assign_issue(
    ...
    let issue =
        features::add_issue_assignees(&database, &tenant, &project, &issue_id, &assignees).await?;
    Response::from_json(&issue)
}

pub(crate) async fn label_issue(
    mut req: Request,
    ctx: crate::request_context::AppRouteContext,
) -> Result<Response> {
    let user = require_auth(&req, &ctx).await?;
    ...
    let issue =
        features::add_issue_labels(&database, &tenant, &project, &issue_id, &labels).await?;
    Response::from_json(&issue)
}

pub(crate) fn issue_string_list(
...
pub(crate) async fn issue_comments(
    ...
    check_project_read_capability(&database, &tenant, &project, user.as_deref(), "issues:read")
        .await?;
    let comments = features::list_comments(&database, &tenant, &project, &issue_id).await?;
...
pub(crate) async fn create_comment(
    ...
    let comment = features::create_comment(
        &database,
        &tenant,
        &project,
        &issue.id,
...
pub(crate) async fn transfer_issue(
    ...
    let issue_id = param(&ctx, "issue_id")?;
    let body: serde_json::Value = req.json().await?;
    let target_tenant = body["tenant"]
        .as_str()
        .or_else(|| body["target_tenant"].as_str())
        .unwrap_or("")
        .trim();
    let target_project = body["project"]
        .as_str()
        .or_else(|| body["target_project"].as_str())
        .unwrap_or("")
        .trim();
    ...
    let before = features::list_issues(&database, &tenant, &project)
        ...
        .find(|issue| issue.id == issue_id || issue.number.to_string() == issue_id)
        .ok_or_else(|| Error::RustError("issue not found".into()))?;
    let issue = features::transfer_issue(
        &database,
        &tenant,
        &project,
        &before.id,
...
pub(crate) async fn delete_issue(
    ...
    .await?;
    if features::delete_issue(&database, &tenant, &project, &issue_id).await? {
...
async fn record_issue_activity(
    ...
    features::create_comment(
        db,
        tenant,
        project,
        issue_id,
        &sty_protocol::TokenPrincipal {
...
fn issue_metadata_messages(
    ...
    if before.workspace != after.workspace {
        match (&before.workspace, &after.workspace) {
            ...
            (Some(previous), Some(next)) => messages.push(format!(
                "changed the linked workspace from {previous} to {next}"
            )),
            _ => {}
        }
    }
