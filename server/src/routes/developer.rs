...
use crate::routes::objects::{
    check_project_capability, check_project_write_capability, check_project_write_role,
    require_auth,
};
...
pub async fn list_project_api_keys(
    ...
    .await?;
    let items = features::list_project_api_keys(&database, &tenant, &project).await?;
...
pub async fn create_project_api_key(
    ...
    .await?;
    let item = features::create_project_api_key(
        &database,
        &tenant,
        &project,
        &user,
...
pub async fn delete_project_api_key(
    ...
    .await?;
    if !features::revoke_project_api_key(&database, &tenant, &project, &id).await? {
...
pub async fn list_project_webhooks(
    ...
    .await?;
    let items = features::list_project_webhooks(&database, &tenant, &project).await?;
    Response::from_json(&paginate_vec(req.url()?, items))
}
...
pub async fn create_project_webhook(
    ...
    .await?;
    if features::active_project_webhook_count(&database, &tenant, &project).await? >= 20 {
        return json_error(429, "project webhook limit reached");
    }
    let item = features::create_project_webhook(
        &database,
        &tenant,
        &project,
        &user,
...
pub async fn delete_project_webhook(
    ...
    .await?;
    if !features::revoke_project_webhook(&database, &tenant, &project, &id).await? {
...
pub async fn test_project_webhook(
    ...
    .await?;
    let Some(hook) = features::project_webhook_by_id(&database, &tenant, &project, &id).await?
    else {
        return json_error(404, "webhook not found");
    };
...
pub async fn list_project_integrations(
    ...
    .await?;
    let items = features::list_project_integrations(&database, &tenant, &project).await?;
    Response::from_json(&paginate_vec(req.url()?, items))
}
...
pub async fn delete_project_integration(
    ...
    .await?;
    if !features::revoke_project_integration(&database, &tenant, &project, &id).await? {
...
pub async fn list_developer_apps(
    ...
    let user = require_auth(&req, &ctx).await?;
    let database = db(&ctx)?;
    let items = features::list_developer_apps(&database, &user).await?;
    Response::from_json(&paginate_vec(req.url()?, items))
}
...
pub async fn create_developer_app(
    ...
    let database = db(&ctx)?;
    let item = features::create_developer_app(
        &database,
        &user,
        name,
        &body.redirect_uri,
...
pub async fn delete_developer_app(
    ...
    let database = db(&ctx)?;
    if !features::revoke_developer_app(&database, &user, &id).await? {
...
pub async fn oauth_app(
    ...
    let database = db(&ctx)?;
    let Some(app) = features::developer_app_by_client_id(&database, &client_id).await? else {
        return json_error(404, "developer app not found");
    };
...
pub async fn oauth_authorize(
    ...
    let code = features::create_oauth_code(
        &database,
        &body.client_id,
        &user,
        &body.tenant,
...
pub async fn oauth_token(
    ...
    let database = db(&ctx)?;
    let Some(grant) = features::exchange_oauth_code(
        &database,
        &body.client_id,
        &body.client_secret,
        &body.code,
