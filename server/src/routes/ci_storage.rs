...
pub(crate) async fn upload_ci_job_artifact(
    ...
    let runner = crate::routes::ci::require_ci_runner(&req, &ctx, true).await?;
    ...
    let database = db(&ctx)?;
    if !features::ci_job_active_for_runner(database, &runner, &job_id).await? {
    ...
    bucket(&ctx.env)?
        ...
        .execute()
        .await?;
    let artifact = features::record_ci_artifact(
        database,
        &job,
        &artifact_name,
        &storage_key,
...
pub(crate) async fn list_ci_job_artifacts(
    ...
    let artifacts = features::list_ci_artifacts(database, &tenant, &project, &job_id)
        .await?
        .into_iter()
        .map(|artifact| {
            json!({
...
pub(crate) async fn download_ci_job_artifact(
    ...
    .await?;
    let Some(artifact) =
        features::ci_artifact_by_id(database, &tenant, &project, &job_id, &artifact_id).await?
    else {
...
pub(crate) async fn restore_ci_cache(
    ...
    let database = db(&ctx)?;
    let Some(record) = features::ci_cache_by_key(database, &tenant, &project, &cache_key).await?
    else {
...
pub(crate) async fn save_ci_cache(
    ...
    bucket(&ctx.env)?
        ...
        .await?;
    let database = db(&ctx)?;
    let record = features::upsert_ci_cache(
        database,
        &tenant,
        &project,
        &cache_key,
...
async fn prune_ci_storage(
    ...
    let artifacts =
        features::stale_ci_artifacts(database, tenant, project, &artifact_cutoff, PRUNE_LIMIT)
            .await?;
    let caches =
        features::stale_ci_caches(database, tenant, project, &cache_cutoff, PRUNE_LIMIT).await?;
    let keys = artifacts
        .iter()
    ...
    for artifact in artifacts {
        features::delete_ci_artifact(database, tenant, project, &artifact.id).await?;
    }
    for cache in caches {
        features::delete_ci_cache(database, tenant, project, &cache.cache_key).await?;
    }
    if !keys.is_empty() {
