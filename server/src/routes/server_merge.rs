use super::prelude::*;
pub(crate) struct ServerMergeSnapshot {
    pub(crate) snapshot_id: Option<String>,
    pub(crate) conflicts: Vec<String>,
    pub(crate) auto_merged_files: usize,
}
...
pub(crate) async fn create_workspace_merge_snapshot(
    env: &Env,
    ...
    let workspace_snapshot =
        read_server_snapshot(&features, tenant, project, workspace_head).await?;
    validate_tree_closure(
        &features,
        database,
        tenant,
        project,
        &workspace_snapshot.root_tree,
    )
    .await?;
    let parent_snapshot = match parent_head {
        Some(id) => Some(read_server_snapshot(&features, tenant, project, id).await?),
        None => None,
    };
    ...
    let base_snapshot_id = match parent_head {
        Some(parent_head) => {
            common_snapshot_ancestor(env, tenant, project, parent_head, workspace_head).await?
        }
        None => None,
    };
    ...
    let base_files = match base_snapshot_id.as_deref() {
        Some(id) => {
            let snapshot = read_server_snapshot(&features, tenant, project, id).await?;
            server_snapshot_blob_map(&features, tenant, project, &snapshot.root_tree).await?
        }
    ...
    let workspace_files =
        server_snapshot_blob_map(&features, tenant, project, &workspace_snapshot.root_tree).await?;
    ...
    let root_tree = write_tree_map(&features, database, tenant, project, &merge.files).await?;
    validate_tree_closure(&features, database, tenant, project, &root_tree).await?;
...
async fn write_tree_map(
    ...
    for dir in ordered_dirs {
        ...
        put_bytes(features, &object_key(tenant, project, &id), bytes.clone()).await?;
        features::record_object(database, tenant, project, &id, "tree", bytes.len()).await?;
...
async fn write_server_merge_snapshot(
    ...
    put_bytes(
        features,
        &object_key(tenant, project, &snapshot_id),
        bytes.clone(),
    )
    .await?;
    features::record_object(
        database,
        tenant,
        project,
        &snapshot_id,
