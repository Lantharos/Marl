...
use crate::features;
use crate::routes::objects::{require_auth, require_web_auth};
use crate::support::{
...
pub async fn create_remote_approval(
    ...
    let approval = features::create_remote_approval(
        &database,
        &user,
        body.action.trim(),
        body.summary.trim(),
...
pub async fn get_remote_approval(
    ...
    let database = db(&ctx)?;
    let Some(approval) = features::remote_approval(&database, &id).await? else {
...
pub async fn approve_remote_approval(
    ...
    let id = param(&ctx, "approval_id")?;
    let database = db(&ctx)?;
    let Some(approval) = features::approve_remote_approval(&database, &id, &user).await? else {
        return json_error(404, "approval not found");
    };
    Response::from_json(&RemoteApprovalStatus {
        id: approval.id,
...
async fn list_account_key_kind(
    ...
    let user = require_auth(&req, &ctx).await?;
    let database = db(&ctx)?;
    let items = features::list_user_keys(&database, &user, kind).await?;
...
async fn create_account_key_kind(
    ...
    features::upsert_user_key(&database, &item).await?;
...
async fn require_signing_key_approval(
    ...
    if matches!(
        features::token_kind(&database, &token).await?.as_deref(),
        Some("web")
    ) {
        return Ok(());
    }
    ...
    match features::consume_remote_approval(&database, approval_id, user, "signing_key_upload")
        .await?
    {
...
fn approval_response(
    req: &Request,
    env: &Env,
    approval: features::RemoteApproval,
...
async fn delete_account_key_kind(
    ...
    let database = db(&ctx)?;
    let Some(item) = features::user_key_by_id(&database, &user, &id).await? else {
        return json_error(404, "key not found");
    };
    if item.kind != kind {
        return json_error(404, "key not found");
    }
    features::revoke_user_key(&database, &user, &id).await?;
...
pub async fn verify_snapshot_id(
    ...
    let known = matches!(
        features::object_kind(database, tenant, project, id).await?,
...
pub async fn validate_snapshot_signature(
    ...
    let key = features::active_signing_key(database, &signature.user, &signature.key_id)
        .await
