...
pub async fn list_ready(
    ...
    for workspace in features::filter_visible_workspaces(
        &database,
        &tenant,
        &project,
        user.as_deref(),
...
pub async fn list_workspace_reviews(
    ...
    let reviews =
        features::list_workspace_reviews(&database, &tenant, &project, &workspace).await?;
...
pub async fn submit_workspace_review(
    ...
    let review = features::submit_workspace_review(
        &database,
        &tenant,
        &project,
        &workspace,
...
pub async fn unmark_ready(
    ...
    features::unmark_workspace_ready(
        &database,
        &tenant,
        &project,
        &workspace,
...
async fn ready_workspace_json(
    ...
    let database = db(ctx)?;
    let marker = features::latest_ready_marker(database, tenant, project, &workspace.name).await?;
    ...
    let approvals = if settings.merge_rules.dismiss_stale_approvals {
        features::current_workspace_approvals(
            database,
            tenant,
            project,
            &workspace.name,
        ...
        features::latest_workspace_approvals(database, tenant, project, &workspace.name).await?
    };
    let stale_approvals = features::stale_workspace_approvals(
        database,
        tenant,
        project,
        &workspace.name,
    ...
    .await?;
    let checks = features::list_workspace_checks(
        database,
        tenant,
        project,
        &workspace.name,
...
async fn snapshot_intents(
    ...
    let bytes = r2_bytes(&features, &object_key(tenant, project, head)).await?;
...
async fn workspace_exists(
    ...
    Ok(features::workspace_states(database, tenant, project)
        .await?
        .into_iter()
...
pub async fn reject_ready(
    ...
    let body: serde_json::Value = req.json().await.unwrap_or_else(|_| json!({}));
    features::reject_workspace_ready(
        &database,
        &tenant,
        &project,
        &workspace,
