...
pub struct MergeRequirementStatus {
    ...
    pub checks: features::WorkspaceCheckSummary,
}
...
pub async fn list_notifications(
    ...
    let notifications = features::list_notifications(&database, &user, limit).await?;
...
pub async fn mark_notification_read(
    ...
    let database = db(&ctx)?;
    if !features::mark_notification_read(&database, &user, &id).await? {
...
pub async fn list_audit_log(
    ...
    .await?;
    let events = features::list_audit_events(
        &database,
        &tenant,
        &project,
...
pub async fn list_workspace_checks(
    ...
    check_workspace_read_capability(&database, &tenant, &project, user.as_deref(), &workspace)
        .await?;
    let Some(state) = features::workspace_state(&database, &tenant, &project, &workspace).await?
    else {
        return json_error(404, "workspace not found");
    };
    let head = query_text(&req, "head").or(state.head);
    let checks =
        features::list_workspace_checks(&database, &tenant, &project, &workspace, head.as_deref())
            .await?;
    Response::from_json(&json!({
        "checks": checks,
        "summary": features::workspace_check_summary(&database, &tenant, &project, &workspace, head.as_deref()).await?,
    }))
}
...
pub async fn submit_workspace_check(
    ...
    let check = features::upsert_workspace_check(
        &database,
        &tenant,
        &project,
        &workspace,
...
pub async fn workspace_merge_status(
    ...
    let stale_approvals =
        features::stale_workspace_approvals(database, tenant, project, workspace, head).await?;
    let checks =
        features::workspace_check_summary(database, tenant, project, workspace, head).await?;
    let unresolved =
        features::unresolved_workspace_comment_count(database, tenant, project, workspace).await?;
...
pub async fn notify_users(
    ...
    for user in users {
        ...
        features::create_notification(database, &user, tenant, project, kind, title, body, href)
            .await?;
    }
    Ok(())
}
...
async fn approvals_for_rules(
    ...
    head: Option<&str>,
    rules: &MergeRules,
) -> Result<Vec<features::WorkspaceReview>> {
    if rules.dismiss_stale_approvals {
        features::current_workspace_approvals(database, tenant, project, workspace, head).await
    } else {
        features::latest_workspace_approvals(database, tenant, project, workspace).await
    }
}
