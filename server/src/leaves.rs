...
pub(crate) async fn get_project_leaf(
    ...
    let database = db(&ctx)?;
    ensure_project_target(&database, &tenant, &project).await?;
    let Some(leaf) = d1::leaf_by_id_or_slug(&database, &tenant, Some(&project), &leaf_id).await?
    else {
...
pub(crate) async fn update_project_leaf(
    ...
    let database = db(&ctx)?;
    let Some(existing) =
        d1::leaf_by_id_or_slug(&database, &tenant, Some(&project), &leaf_id).await?
    else {
        return json_error(404, "leaf not found");
    };
    ...
    let leaf = d1::update_leaf(&database, &tenant, Some(&project), &leaf_id, patch).await?;
    Response::from_json(&leaf)
}
...
pub(crate) async fn delete_project_leaf(
    ...
    ensure_project_leaf_write(&database, &tenant, &project, &user, &existing).await?;
    d1::delete_leaf(&database, &tenant, Some(&project), &leaf_id).await?;
...
pub(crate) async fn create_tenant_leaf(
    ...
    let leaf = d1::create_leaf(&database, &tenant, None, &TokenPrincipal { user }, input).await?;
    Response::from_json(&leaf)
}
...
pub(crate) async fn get_tenant_leaf(
    ...
    let leaf_id = param(&ctx, "leaf")?;
    let database = db(&ctx)?;
    let Some(leaf) = d1::leaf_by_id_or_slug(&database, &tenant, None, &leaf_id).await? else {
        return json_error(404, "leaf not found");
    };
...
pub(crate) async fn update_tenant_leaf(
    ...
    let leaf_id = param(&ctx, "leaf")?;
    let database = db(&ctx)?;
    let Some(existing) = d1::leaf_by_id_or_slug(&database, &tenant, None, &leaf_id).await? else {
        return json_error(404, "leaf not found");
    };
    ...
    let leaf = d1::update_leaf(&database, &tenant, None, &leaf_id, patch).await?;
    Response::from_json(&leaf)
}
...
pub(crate) async fn delete_tenant_leaf(
    ...
    ensure_tenant_leaf_write(&database, &tenant, &user, &existing).await?;
    d1::delete_leaf(&database, &tenant, None, &leaf_id).await?;
    Response::from_json(&OkResponse { ok: true })
}
...
pub(crate) async fn visible_project_leaves(
    ...
    for leaf in d1::list_leaves(database, tenant, Some(project)).await? {
...
async fn ensure_project_target(
    ...
    if !d1::tenant_exists(database, tenant).await?
        || !d1::project_exists(database, tenant, project).await?
    {
...
async fn ensure_tenant_write(
    ...
    let role = d1::tenant_effective_role(database, tenant, user).await?;
    if d1::role_allows(role.as_deref(), minimum) {
        return Ok(());
    }
...
async fn ensure_project_leaf_write(
    ...
    let role = d1::project_effective_role(database, tenant, project, user).await?;
    if leaf.author == user || d1::role_allows(role.as_deref(), "maintainer") {
        check_project_write_capability(
            database,
            tenant,
            project,
...
async fn can_read_project_leaf(
    ...
    if leaf.visibility == "private" {
        let role = d1::project_effective_role(database, tenant, project, user).await?;
        return Ok(leaf.author == user || d1::role_allows(role.as_deref(), "maintainer"));
    }
    Ok(true)
}
...
async fn visible_tenant_leaves(
    ...
    let mut visible = Vec::new();
    for leaf in d1::list_leaves(database, tenant, None).await? {
