...
CREATE INDEX IF NOT EXISTS idx_ci_runners_project ON ci_runners(tenant, project, disabled_at, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_ci_jobs_project ON ci_jobs(tenant, project, workspace, head, queued_at DESC);
CREATE INDEX IF NOT EXISTS idx_ci_jobs_queue ON ci_jobs(tenant, project, status, queued_at);
...
CREATE INDEX IF NOT EXISTS idx_ci_job_logs_job ON ci_job_logs(job_id, seq);
